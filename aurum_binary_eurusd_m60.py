...
Feed FX -> contesto cross-pair + news -> feature causali -> agenti /
AURUM BURST-15 -> segnali binari M60 PAPER-ONLY con trigger e countdown verificati
lato motore, tutto registrato su database e riaddestrato da solo.

    TWELVE_DATA_API_KEY=... ALPHAVANTAGE_API_KEY=... python3 aurum_binary_eurusd_m60.py run --payout 0.8
                                                   # EUR/USD, CALL/PUT a 60s
...
ESECUZIONE: il motore resta PAPER-ONLY. E' presente un bridge Pocket separato
per mostrare lo stato/saldo fornito da un connettore esterno supportato e per
preparare la puntata dal frontend, ma questo file non include endpoint privati,
cookie o reverse engineering di Pocket Option.

Cosa c'e' dentro, rispetto al progetto completo:
  * feed Twelve Data FX real-time per EUR/USD + paniere di 28 cross di contesto;
  * Alpha Vantage NEWS_SENTIMENT opzionale per macro, banche centrali e mercati;
  * feed Binance/Coinbase legacy, simulatore offline e replay da CSV;
...
class Config:
    ...
    # ---------------------------------------------------------------- venue
    source: str = "twelvedata"       # twelvedata | binance | coinbase | sim | csv
    symbol: str = "EUR/USD"
    ...
    # ---------------------------------------------------------- FX / contesto
    # Twelve Data fornisce tick real-time via WebSocket. La chiave non viene
    # mai salvata nel database o esposta nel frontend: usa TWELVE_DATA_API_KEY.
    twelve_data_api_key: str | None = None
    # 28 cross principali fra EUR, USD, GBP, JPY, CHF, CAD, AUD, NZD.
    # EUR/USD e' l'asset operativo; gli altri entrano SOLO come contesto.
    fx_context_symbols: str = (
    ...
    # --------------------------------------------------------- news / macro
    # Alpha Vantage NEWS_SENTIMENT e' opzionale. Senza chiave il motore continua
    # a funzionare, ma dichiara news_available=0 invece di inventare sentiment.
    news_enabled: bool = True
    alpha_vantage_api_key: str | None = None
    news_poll_s: float = 300.0
    news_limit: int = 100
    ...
    # -------------------------------------------------------- Pocket bridge
    # Pocket Option non espone una API diretta pubblica documentata. Questo e'
    # quindi un contratto LOCALE/esterno: GET /status puo' fornire saldo e payout.
    # Nessun endpoint privato Pocket e nessuna credenziale vengono codificati qui.
    pocket_bridge_url: str | None = None
    pocket_bridge_token: str | None = None
    #: Ogni quanto interrogare il bridge, da un thread suo. Non e' un dettaglio
    #: di prestazioni: la lettura NON deve mai stare sul percorso di una
    #: risposta HTTP, o la dashboard si blocca quando il bridge non risponde.
    pocket_bridge_poll_s: float = 5.0
    ...
    def counts(self) -> dict[str, int]:
        ...
        try:
            ...
            for table in (
                ...
                "shadow_decisions", "model_versions", "wallet_ledger", "learning_state",
            ):
...
class Feed:
    ...
    def state(self) -> dict[str, Any]:
        return {
            ...
            "has_real_spread": self.has_real_spread,
        }


class TwelveDataFxFeed(Feed):
    """Feed FX real-time Twelve Data.

    Una sola connessione sottoscrive EUR/USD e il paniere di cross di contesto.
    Il WebSocket /quotes/price espone il prezzo tick ma non bid/ask/volume: per
    questo `has_real_spread=False` e `supports_trades=False`. Il motore NON
    inventa order-flow o spread; gli agenti relativi si astengono.
    """
    ...
    def __init__(self, cfg: Config) -> None:
        super().__init__(cfg)
        self.api_key = cfg.twelve_data_api_key or os.environ.get("TWELVE_DATA_API_KEY")
        ...
        raw = [x.strip().upper() for x in cfg.fx_context_symbols.split(",") if x.strip()]
        target = self._norm(cfg.symbol)
        self.symbol = target
        self.symbols = list(dict.fromkeys([target] + [self._norm(x) for x in raw]))
        self._series: dict[str, deque[tuple[int, float]]] = {
            sym: deque(maxlen=1200) for sym in self.symbols
        }
    ...
    def ws_url(self) -> str:
        if not self.api_key:
            return self.WS_BASE + "?apikey=MISSING"
        from urllib.parse import quote
        return self.WS_BASE + "?apikey=" + quote(self.api_key, safe="")

    def _connect(self) -> None:
        if not self.api_key:
            self.connected = False
            self.last_error = "TWELVE_DATA_API_KEY mancante"
            return
    ...
    def poll(self, timeout: float) -> list[Any]:
        ...
        if time.time() - self._last_heartbeat >= 10.0:
            try:
                self.ws.send_text(json.dumps({"action": "heartbeat"}))
                self._last_heartbeat = time.time()
            except Exception:
                pass
        try:
        ...
        for text in raw:
            ...
            try:
                ...
                continue
            etype = str(msg.get("event") or msg.get("type") or "").lower()
            if "subscribe" in etype:
                for key in ("success", "subscribed", "symbols"):
                    val = msg.get(key)
                    if isinstance(val, list):
                        self.subscribed.update(self._norm(str(x)) for x in val)
                failed = msg.get("fails") or msg.get("failed") or []
                if isinstance(failed, list):
                    self.failed_symbols.update(self._norm(str(x)) for x in failed)
                continue
            if etype != "price" and "price" not in msg:
                continue
            try:
                sym = self._norm(str(msg.get("symbol") or ""))
                price = float(msg["price"])
                raw_ts = float(msg.get("timestamp") or time.time())
                ets = int(raw_ts * 1000) if raw_ts < 10_000_000_000 else int(raw_ts)
            except (TypeError, ValueError, KeyError):
                continue
            if not sym or price <= 0:
                continue
            ts = now_ms()
            self.measure_latency(ets)
            self._latest[sym] = (ts, price)
            q = self._series.setdefault(sym, deque(maxlen=1200))
            q.append((ts, price))
            if sym == self.symbol:
                # WebSocket price non fornisce bid/ask. Usiamo lo stesso prezzo
                # su entrambi i lati SOLO per trasportare il mid canonico; i
                # gate di spread vengono disattivati per questo adapter.
                out.append(Tick(
                    ts=ts, exchange=self.name, symbol=sym,
                    bid_price=price, bid_qty=0.0,
                    ask_price=price, ask_qty=0.0,
                    last_price=price, latency_ms=self.last_latency_ms,
                ))
        return out

    def _return(self, sym: str, horizon_ms: int) -> float | None:
        q = self._series.get(sym)
        if not q or len(q) < 2:
            return None
        ts, px = q[-1]
        cutoff = ts - horizon_ms
        old = None
        ...
        if old is None or old <= 0:
            return None
        return (px - old) / old * 10_000.0

    def context_features(self) -> dict[str, Any]:
        """Feature cross-FX causali; mai segnali autonomi su altri asset."""
        out: dict[str, Any] = {}
        now = now_ms()
        fresh = {s for s, (t, _) in self._latest.items() if now - t <= 5000}
        out["fxctx_available"] = 1.0 if self.symbol in fresh else 0.0
        out["fxctx_pairs_total"] = float(len(self.symbols))
        out["fxctx_pairs_fresh"] = float(len(fresh))
        out["fxctx_breadth"] = len(fresh) / max(len(self.symbols), 1)

        currencies = ("EUR", "USD", "GBP", "JPY", "CHF", "CAD", "AUD", "NZD")
        scores10 = {c: [] for c in currencies}
        scores60 = {c: [] for c in currencies}
        target60 = None
        for sym in self.symbols:
            if "/" not in sym:
                continue
            base, quote = sym.split("/", 1)
            r10 = self._return(sym, 10_000)
            r60 = self._return(sym, 60_000)
            key = sym.replace("/", "_").lower()
            out[f"fxctx_{key}_r10_bps"] = r10
            out[f"fxctx_{key}_r60_bps"] = r60
            if sym == self.symbol:
                target60 = r60
            for r, bucket in ((r10, scores10), (r60, scores60)):
                if r is None:
                    continue
                if base in bucket:
                    bucket[base].append(r)
                if quote in bucket:
                    bucket[quote].append(-r)
        for c in currencies:
            a = scores10[c]
            b = scores60[c]
            out[f"fxctx_{c.lower()}_strength_10s"] = sum(a)/len(a) if a else None
            out[f"fxctx_{c.lower()}_strength_60s"] = sum(b)/len(b) if b else None
        eur = out.get("fxctx_eur_strength_60s")
        usd = out.get("fxctx_usd_strength_60s")
        out["fxctx_eurusd_relative_strength_60s"] = (
            eur - usd if eur is not None and usd is not None else target60
        )
        return out
    ...
    def state(self) -> dict[str, Any]:
        ...
        d.update({
            "target": self.symbol,
            "context_pairs": len(self.symbols),
            "subscribed_reported": len(self.subscribed),
...
class SyntheticFeed(Feed):
    ...
    #: Prezzo di partenza per scala del tick. Un simulatore che genera 100.000
    #: mentre la configurazione dichiara EUR/USD con tick 0,00001 non prova
    #: NIENTE di utile su questa build: il movimento atteso diventa dieci
    #: miliardi di tick, il cancello sulle finestre ferme non scatta mai, e la
    #: dashboard stampa un cambio a cinque decimali su un prezzo da criptovaluta.
    #: Il simulatore deve simulare lo strumento configurato.
    START_BY_TICK = ((1e-5, 1.08500), (1e-4, 150.000), (1e-2, 100_000.0))
    #: E la stessa cosa vale per la VOLATILITA'. Con il valore da criptovaluta
    #: il simulatore muoveva EUR/USD di 400 bps in sei minuti - piu' di quanto
    #: il cambio faccia in una giornata. Girare cosi' non prova che i cancelli
    #: funzionino sul cambio vero: li prova su uno strumento che non esiste.
    #: Sigma per passo, in punti base.
    VOL_BY_TICK = ((1e-5, 0.35), (1e-4, 1.2), (1e-2, 4.0))

    def __init__(self, cfg: Config) -> None:
        super().__init__(cfg)
        self.rng = random.Random(cfg.sim_seed)
        self.price = next((p for t, p in self.START_BY_TICK
                           if cfg.tick_size <= t), 100_000.0)
        self.drift = 0.0
        self.vol_bps = next((v for t, v in self.VOL_BY_TICK
                             if cfg.tick_size <= t), 4.0)
    ...
    def _step(self, dt: float) -> None:
        ...
        sigma = self.price * (self.vol_bps / 10_000.0) * math.sqrt(dt)
...
class MarketData:
    ...
    def data_quality(self) -> dict[str, Any]:
        ...
        if self.feed.has_real_spread:
            ...
            notes.append("bid/ask non disponibile: filtro spread non applicabile")
...
class FeatureEngine:
    ...
    def __init__(self, cfg: Config, market: MarketData,
                 external_features: Callable[[], dict[str, Any]] | None = None) -> None:
        self.cfg = cfg
        self.market = market
    ...
    def compute(self, ts: int | None = None) -> dict[str, Any] | None:
        ...
        f["flow_expected"] = bool(self.market.feed.supports_trades)
        f["spread_expected"] = bool(self.market.feed.has_real_spread)
        ...
        if self.external_features is not None:
            try:
                ext = self.external_features() or {}
                for k, v in ext.items():
                    if k not in f:
                        f[k] = v
            except Exception as exc:
                f["external_context_error"] = str(exc)
...
class DecisionEngine:
    ...
    def decide(self, vector: dict, market: dict, health: dict) -> Decision:
        ...
        if bool(f.get("spread_expected", True)):
        ...
        # decidere mai.
        needed = ["return_1000ms"]
        if bool(f.get("flow_expected", True)):
            needed.append("volume_imbalance_1s")
        if book_expected:
        ...
        # In live FX il contesto dichiarato deve esserci davvero. Se il provider
        # ha accettato poche coppie o la news API e' assente, NO TRADE invece di
        # una previsione costruita su input mancanti. Replay/simulatore restano
        # indipendenti da API esterne.
        if cfg.source == "twelvedata":
            breadth = f.get("fxctx_breadth")
            if cfg.require_fx_context and (breadth is None or float(breadth) < cfg.fx_context_min_breadth):
                reasons.append(
                    f"contesto FX incompleto ({float(breadth or 0.0):.0%} < "
                    f"{cfg.fx_context_min_breadth:.0%})"
                )
            if cfg.news_enabled and cfg.require_news_context and not bool(f.get("news_available")):
                reasons.append("news macro/sentiment non disponibili")
        ...
        if (news_risk is not None and float(news_risk) >= cfg.news_block_risk and
                news_age is not None and float(news_age) <= cfg.news_block_minutes * 60.0):
            reasons.append(
                f"news ad alto impatto ({float(news_risk):.2f}) negli ultimi "
                f"{float(news_age):.0f}s"
            )
      ...
      <div>
        ...
        <div class="cells" style="grid-template-columns:repeat(4,1fr);margin-top:14px">
          <div class="cell"><div class="k">Puntata</div>
            <div class="v tnum" id="wal-stake">—</div>
            <div style="display:flex;gap:5px;margin-top:6px">
              <input id="stake-input" type="number" min="0.01" step="0.01"
                style="width:76px;background:#111827;color:#e5e7eb;border:1px solid #334155;border-radius:5px;padding:4px"/>
              <button onclick="setStake()" style="padding:4px 7px">Applica</button>
            </div></div>
...
<div class="grid g3 mb">
  ...
  <div class="panel">
    <div class="ph">
      <div><h2>Contesto FX</h2>
        <div class="sub" id="fx-sub">forza delle valute dai cross</div></div>
      <span class="pill" id="fx-state">—</span>
    </div>
    <div class="pb" id="fx-body"><div class="empty">in attesa</div></div>
  </div>

  <div class="panel">
    <div class="ph">
      <div><h2>Macro &amp; news</h2>
        <div class="sub" id="news-sub">il rischio evento domina il micro</div></div>
      <span class="pill" id="news-state">—</span>
    </div>
    <div class="pb" id="news-body"><div class="empty">in attesa</div></div>
  </div>

  <div class="panel">
    <div class="ph">
      <div><h2>Bridge broker</h2>
        <div class="sub">sola lettura &middot; nessun ordine inviato</div></div>
      <span class="pill" id="br-state">—</span>
    </div>
    <div class="pb" id="br-body"><div class="empty">in attesa</div></div>
  </div>
...
// I decimali di un PREZZO dipendono dallo strumento, non da un default.
// Su EUR/USD il tick e' 0,00001: con due decimali 1,08532 diventa "1,09" e
// ingresso e uscita di un'operazione sembrano identici - il movimento
// spariva dallo schermo, comprese le vincite. I decimali si ricavano dal
// tick vero, che il motore dichiara in /config.
var PRICE_DECIMALS = 2, PRICE_UNIT = "";
function priceDecimals(tick){
  if(!tick || !isFinite(tick) || tick <= 0) return 2;
  // due cifre in piu' del tick, cosi' meta' tick resta visibile
  return Math.min(8, Math.max(2, Math.ceil(-Math.log10(tick))));
}
function px(v){ return v===null||v===undefined||isNaN(v)
  ? "—" : PRICE_UNIT + num(v, PRICE_DECIMALS); }
function axisDecimals(step){
  if(!step || !isFinite(step) || step <= 0) return PRICE_DECIMALS;
...
function setStake(){
  var el=document.getElementById("stake-input"), v=Number(el&&el.value);
  if(!isFinite(v)||v<=0){ alert("Inserisci una puntata valida"); return; }
  post("/settings/stake",{stake_amount:v}).then(function(){ tickSlow(); })
    .catch(function(e){ alert("Puntata non aggiornata: "+e); });
}
...
function drawChart(){
  ...
  for(var p=Math.ceil(lo/step)*step; p<hi; p+=step){
    ...
    // I decimali dell'asse vengono dal PASSO della griglia: con un passo di
    // 0,0001 su EUR/USD, due decimali stampavano sei volte "1,09".
    cx.fillText(p.toFixed(axisDecimals(step)), W-padR+7, yy);
  }
  ...
  if(state.lastPrice){
    ...
    cx.fillText(state.lastPrice.toFixed(PRICE_DECIMALS), W-padR+6, ly);
  }
...
function renderWallet(w){
  ...
  var si=$("stake-input"); if(si && document.activeElement!==si) si.value=Number(w.desired_stake||w.next_stake||0).toFixed(2);
  if(w.pocket && w.pocket.connected && w.pocket.balance!==null && w.pocket.balance!==undefined){
    $("wal-sub").textContent += " · Pocket " + num(w.pocket.balance) + " " + (w.pocket.currency||cur);
  }
...
function renderSignal(s, d){
  ...
  var html = '<div class="sig"><div class="arrow '+cls+'">'
    ...
    + '<div class="cell"><div class="k">'
      ...
      + '<div class="v tnum">'+px(atMarket?sig.reference_price:sig.trigger_price)+'</div></div>'
    + '<div class="cell"><div class="k">Ingresso</div><div class="v tnum">'
      + px(sig.entry_price) + '</div></div>'
  ...
  $("history").innerHTML = '<table><thead><tr>'
    ...
    + rows.map(function(t){
      ...
      return '<tr><td class="tnum muted">'+hhmm(t.triggered_at||t.ts)+'</td>'
        ...
        + '<td class="n">'+px(t.entry_price)+'</td>'
        + '<td class="n">'+px(t.expiry_price)+'</td>'
...
function tickFast(){
  Promise.all([get("/diagnostics"), get("/signals/current"), get("/market"),
    ...
    .then(function(r){
      ...
      $("price").textContent = px(mk.price);
      state.lastPrice = mk.price;
      $("p-sym").textContent = (mk.symbol||"");
...
// e USD scende contro tutti, EUR/USD ha un contesto, non solo un tick. Qui si
// mostra quel contesto, perche' e' cio' che distingue un movimento del cambio
// da un movimento di UNA delle due valute.
function renderFx(fx){
  var st = $("fx-state");
  var avail = Number(fx.fxctx_available||0) > 0;
  var fresh = Number(fx.fxctx_pairs_fresh||0), tot = Number(fx.fxctx_pairs_total||0);
  st.textContent = avail ? (fresh + "/" + tot + " cross vivi") : "non disponibile";
  st.className = "pill " + (avail ? "on" : "");
  $("fx-sub").textContent = avail
    ? ("ampiezza " + pct(fx.fxctx_breadth||0) + " \u00b7 forza dai cross")
    : "il feed non manda il paniere: gli agenti di contesto si astengono";

  // Forza per valuta, se il motore la calcola.
  var strengths = [];
  Object.keys(fx).forEach(function(k){
    var m = k.match(/^fxctx_([a-z]{3})_strength_60s$/i);
    if(m && fx[k] !== null && fx[k] !== undefined)
      strengths.push([m[1].toUpperCase(), Number(fx[k])]);
  });
  strengths.sort(function(a,b){ return b[1]-a[1]; });

  // I cross piu' mossi negli ultimi 60 secondi.
  var moves = [];
  Object.keys(fx).forEach(function(k){
    var m = k.match(/^fxctx_([a-z]{3})_([a-z]{3})_r60_bps$/i);
    if(m && fx[k] !== null && fx[k] !== undefined)
      moves.push([m[1].toUpperCase()+"/"+m[2].toUpperCase(), Number(fx[k])]);
  });
  moves.sort(function(a,b){ return Math.abs(b[1])-Math.abs(a[1]); });

  var html = "";
  // La grandezza che riassume tutto: quanto EUR e' forte MENO quanto lo e' USD.
  // Se il cambio sale ma la forza relativa e' piatta, sta salendo per una sola
  // delle due gambe - un contesto diverso, e va visto.
  var rel = fx.fxctx_eurusd_relative_strength_60s;
  if(rel !== null && rel !== undefined){
    html += '<div class="kv" style="margin-bottom:8px"><span><b>Forza relativa 60s</b></span>'
      + '<span class="n tnum '+(Number(rel)>=0?"up":"down")+'">'
      + (Number(rel)>=0?"+":"")+Number(rel).toFixed(2)+' bps</span></div>';
  }
  if(strengths.length){
    var maxAbs = Math.max.apply(null, strengths.map(function(x){ return Math.abs(x[1]) || 0; })) || 1;
    html += '<div class="sub" style="margin-bottom:6px">Forza valute (60s)</div>';
    html += strengths.slice(0,8).map(function(x){
      var w = Math.min(100, Math.abs(x[1])/maxAbs*100);
      var cls = x[1] >= 0 ? "up" : "down";
      return '<div class="kv"><span>'+esc(x[0])+'</span>'
        + '<span style="display:flex;align-items:center;gap:8px">'
        + '<span class="bar" style="width:70px;margin-top:0"><i class="'+cls+'" style="width:'+w+'%;background:currentColor"></i></span>'
        + '<span class="n tnum '+cls+'">'+(x[1]>=0?"+":"")+x[1].toFixed(2)+'</span></span></div>';
    }).join("");
  }
  if(moves.length){
    html += '<div class="sub" style="margin:10px 0 6px">Cross piu\u2019 mossi (60s, bps)</div>';
    html += moves.slice(0,5).map(function(x){
      return '<div class="kv"><span>'+esc(x[0])+'</span>'
        + '<span class="n tnum '+(x[1]>=0?"up":"down")+'">'
        + (x[1]>=0?"+":"")+x[1].toFixed(2)+'</span></div>';
    }).join("");
  }
  $("fx-body").innerHTML = html ||
    '<div class="empty">nessun cross aggiornato: senza paniere il contesto non '
    + 'e\u2019 calcolabile e non viene inventato</div>';
}

function renderNews(nw){
  var st = $("news-state");
  var on = !!nw.enabled, key = !!nw.api_key_configured;
  var avail = Number(nw.news_available||0) > 0;
  if(!on){ st.textContent = "spente"; st.className = "pill"; }
  else if(!key){ st.textContent = "chiave mancante"; st.className = "pill warn"; }
  else if(avail){ st.textContent = "attive"; st.className = "pill on"; }
  else { st.textContent = "in attesa"; st.className = "pill warn"; }

  var risk = nw.news_event_risk, sent = nw.news_sentiment;
  var age = nw.news_latest_age_s;
  $("news-sub").textContent = avail
    ? ("ultimo articolo " + (age!=null ? Math.round(age)+"s fa" : "—"))
    : "senza news il rischio evento non e\u2019 misurabile";

  var rows = [
    ["Rischio evento", risk!=null ? pct(risk) : "—",
      risk!=null && risk>=0.7 ? "down" : (risk!=null && risk>=0.4 ? "warn" : "up")],
    ["Sentiment", sent!=null ? (sent>=0?"+":"")+Number(sent).toFixed(3) : "—",
      sent!=null ? (sent>=0?"up":"down") : ""],
    ["Articoli", nw.news_articles!=null ? String(Math.round(nw.news_articles)) : "—", ""],
    ["Rilevanti EUR/USD", nw.news_relevant_articles!=null
      ? String(Math.round(nw.news_relevant_articles)) : "—", ""],
  ];
  var html = rows.map(function(r){
    return '<div class="kv"><span>'+esc(r[0])+'</span>'
      + '<span class="n tnum '+(r[2]||"")+'">'+r[1]+'</span></div>';
  }).join("");
  if(risk!=null && Number(risk) >= 0.7){
    html += '<div class="empty warn" style="margin-top:8px">rischio evento alto: '
      + 'su una binaria da 60 secondi il salto batte qualsiasi edge '
      + 'microstrutturale, quindi il motore non opera</div>';
  }
  if(nw.last_error){
    html += '<div class="empty warn" style="margin-top:8px">news: '
      + esc(nw.last_error) + '</div>';
  }
  $("news-body").innerHTML = html;
}

function renderBridge(br){
  var st = $("br-state");
  if(!br.configured){ st.textContent = "non configurato"; st.className = "pill"; }
  else if(br.connected && !br.stale){ st.textContent = "collegato"; st.className = "pill on"; }
  else if(br.stale){ st.textContent = "dato vecchio"; st.className = "pill warn"; }
  else { st.textContent = "non connesso"; st.className = "pill off"; }

  var rows = [
    ["Saldo broker", br.balance!=null ? num(br.balance)+" "+(br.currency||"") : "—", ""],
    ["Payout", br.payout!=null ? pct(br.payout,0) : "—", ""],
    ["Asset", br.asset || "—", ""],
    ["Conto", br.mode || br.account_type || "—", ""],
    ["Ultima lettura", br.age_s!=null ? Math.round(br.age_s)+"s fa" : "mai",
      br.stale ? "warn" : ""],
    ["Ordini inviati", String(br.orders_sent||0), "up"],
  ];
  var html = rows.map(function(r){
    return '<div class="kv"><span>'+esc(r[0])+'</span>'
      + '<span class="n tnum '+(r[2]||"")+'">'+esc(r[1])+'</span></div>';
  }).join("");
  html += '<div class="sub" style="margin-top:8px">'
    + '<b>Sola lettura.</b> In questo programma non esiste una chiamata capace '
    + 'di piazzare un ordine, e una verifica del selftest legge il sorgente per '
    + 'garantirlo.</div>';
  if(br.last_error){
    html += '<div class="empty warn" style="margin-top:8px">bridge: '
      + esc(br.last_error) + '</div>';
  }
  $("br-body").innerHTML = html;
}
...
function tickSlow(){
  ...
  get("/news").then(renderNews).catch(function(){
    $("news-state").textContent = "non raggiungibile";
    $("news-state").className = "pill off"; });
  get("/pocket").then(renderBridge).catch(function(){
    $("br-state").textContent = "non raggiungibile";
    $("br-state").className = "pill off"; });
...
// Prima di disegnare qualunque prezzo bisogna sapere COME si scrive: il tick
// dello strumento arriva da /config. Finche' non arriva si resta sul default a
// due decimali, che e' giusto per le cripto e sbagliato per il forex - per
// questo la configurazione si legge subito, prima del primo disegno.
get("/config").then(function(cfg){
  PRICE_DECIMALS = priceDecimals(cfg.tick_size);
  PRICE_UNIT = /USD(T)?$/.test(String(cfg.symbol||"").replace("/",""))
               && Number(cfg.tick_size) >= 0.001 ? "$" : "";
  $("p-sym").textContent = cfg.symbol || "—";
  loadCandles(); tickFast(); tickSlow();
}).catch(function(){ loadCandles(); tickFast(); tickSlow(); });
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                elif path == "/settings/stake":
                    body = self._json_body()
                    try:
                        amount = float(body.get("stake_amount"))
                    except (TypeError, ValueError):
                        self._send({"error": "stake_amount mancante o non "
                                             "numerico"}, 400)
                        return
                    # Un limite superiore serve davvero: una puntata enorme non
                    # e' un errore di battitura innocuo, rende il saldo
                    # incapace di coprirla e BRUCIA il ciclo all'istante,
                    # facendo ripartire lo studio. Una POST non deve poterlo
                    # fare per sbaglio.
                    cap = max(1.0, float(engine.cfg.wallet_start))
                    if not math.isfinite(amount) or amount <= 0:
                        self._send({"error": "stake_amount deve essere > 0"}, 400)
                        return
                    if amount > cap:
                        self._send({"error": f"stake_amount oltre il capitale "
                                             f"del ciclo ({cap:.2f})"}, 400)
                        return
                    # Non cambiamo retroattivamente le operazioni gia' aperte.
                    engine.cfg.stake_mode = "fixed"
                    engine.cfg.stake_amount = round(amount, 2)
                    self._send({"ok": True, "stake_amount": engine.cfg.stake_amount,
                                "wallet": engine.signals.wallet.status(),
                                "pocket": engine.pocket.status()})
        ...
        def _route(self, path: str):
            ...
            if path == "/fx-context":
                fn = getattr(engine.feed, "context_features", None)
                return (fn() if callable(fn) else {"available": False}),
...
class MacroNewsScanner:
    """Snapshot macro/news non bloccante, aggiornato in un thread separato."""

    HIGH_IMPACT = (
        "interest rate", "rate decision", "central bank", "ecb", "federal reserve",
        "fed ", "powell", "lagarde", "cpi", "inflation", "nonfarm", "payroll",
        "employment", "gdp", "recession", "tariff", "sanction", "war", "intervention",
    )

    def __init__(self, cfg: Config) -> None:
        self.cfg = cfg
        self.api_key = cfg.alpha_vantage_api_key or os.environ.get("ALPHAVANTAGE_API_KEY")
        self._lock = threading.Lock()
        self._snapshot: dict[str, Any] = {
            "news_available": 0.0, "news_sentiment": None, "news_event_risk": None,
            "news_latest_age_s": None, "news_articles": 0.0,
        }
        self._running = False
        self._thread: threading.Thread | None = None
        self.last_error: str | None = None
    ...
    def start(self) -> None:
        if not self.cfg.news_enabled or not self.api_key or self._running:
            return
        self._running = True
        self._thread = threading.Thread(target=self._loop, name="macro-news", daemon=True)
        self._thread.start()
    ...
    def _loop(self) -> None:
        while self._running:
            try:
                ...
                self.last_error = f"{type(exc).__name__}: {exc}"
            for _ in range(max(1, int(self.cfg.news_poll_s * 2))):
    ...
    def refresh(self) -> dict[str, Any]:
        ...
        params = {
            ...
            "sort": "LATEST",
            "limit": str(max(10, min(1000, int(self.cfg.news_limit)))),
            "apikey": self.api_key,
        }
        ...
        if not isinstance(feed, list):
            msg = payload.get("Note") or payload.get("Information") if isinstance(payload, dict) else None
            raise RuntimeError(msg or "risposta NEWS_SENTIMENT senza feed")
        ...
        for a in feed:
            ...
            # EUR/USD: diamo piu' peso agli articoli che citano le due valute o
            # le rispettive banche centrali, ma manteniamo il macro globale.
            relevance = 1.0
            if any(x in text for x in ("euro", "eur", "ecb", "lagarde")):
                relevance += 0.75
            if any(x in text for x in ("dollar", "usd", "federal reserve", "fed ", "powell")):
                relevance += 0.75
            try:
                sent = float(a.get("overall_sentiment_score") or 0.0)
            except (TypeError, ValueError):
                sent = 0.0
            ts = self._parse_time(a.get("time_published"))
            age_s = max(0.0, (now-ts)/1000.0) if ts else 86_400.0
            freshness = math.exp(-age_s / 7200.0)
            w = relevance * freshness
            weighted += sent * w
            wsum += w
            if relevance > 1.0:
                relevant += 1
            if latest_age is None or age_s < latest_age:
                latest_age = age_s
            impact_hits = sum(1 for k in self.HIGH_IMPACT if k in text)
            if impact_hits:
                # rischio 0..1: piu' keyword + maggiore freschezza = piu' alto.
                local = min(1.0, 0.55 + 0.12 * impact_hits) * math.exp(-age_s/1800.0)
                risk = max(risk, local)

        snap = {
            "news_available": 1.0,
            "news_sentiment": weighted/wsum if wsum else 0.0,
            "news_event_risk": risk,
            "news_latest_age_s": latest_age,
            "news_articles": float(len(feed)),
            "news_relevant_articles": float(relevant),
            "news_updated_ts": now,
        }
        with self._lock:
            self._snapshot = snap
    ...
    def status(self) -> dict[str, Any]:
        return {**self.snapshot(), "enabled": self.cfg.news_enabled,
                "api_key_configured": bool(self.api_key), "last_error": self.last_error}


def _bridge_health(st: dict[str, Any]) -> dict[str, Any]:
    """Un solo `status()` per risposta: prima ne venivano chiamati due, e
    quando erano ancora una GET di rete costavano il doppio del timeout."""
    if not st.get("configured"):
        state = "DISABLED"
    elif st.get("connected") and not st.get("stale"):
        state = "UP"
    else:
        state = "DEGRADED"
    return {"status": state, "detail": st}


class PocketBridge:
    """Bridge dichiarativo verso un connettore esterno/supportato.

    Contratto opzionale del bridge:
      GET  <url>/status -> {connected,balance,currency,payout,asset,mode}
    Non contiene login, cookie, WebSocket privati o reverse engineering Pocket.
    """

    def __init__(self, cfg: Config) -> None:
        self.cfg = cfg
        self.url = (cfg.pocket_bridge_url or os.environ.get("POCKET_BRIDGE_URL") or "").rstrip("/")
        self.token = cfg.pocket_bridge_token or os.environ.get("POCKET_BRIDGE_TOKEN")
        self.last_error: str | None = None
        self.last_status: dict[str, Any] = {}
        self.last_fetch_ts: int = 0
        self.fetches: int = 0
        self._lock = threading.Lock()
        self._running = False
        self._thread: threading.Thread | None = None
    ...
    # ------------------------------------------------------------- ciclo
    def start(self) -> None:
        """Il bridge si interroga da un thread suo.

        `status()` faceva una GET di rete con timeout 3s, e veniva chiamata
        DENTRO le risposte HTTP: `/health` ne conteneva due, quindi una sola
        richiesta poteva bloccare sei secondi - su una dashboard che interroga
        `/health` ogni secondo. Il costo di rete adesso sta qui, fuori dal
        percorso delle richieste, e chi legge vede sempre l'ultimo stato noto
        con la sua eta'.
        """
        if not self.url or self._running:
            return
        self._running = True
        self._thread = threading.Thread(target=self._loop, name="bridge",
                                        daemon=True)
        self._thread.start()
    ...
    def _loop(self) -> None:
        while self._running:
            try:
                self.refresh()
            except Exception as exc:  # noqa: BLE001 - il bridge non ferma nulla
                self.last_error = f"{type(exc).__name__}: {exc}"
            for _ in range(int(max(1.0, self.cfg.pocket_bridge_poll_s) * 2)):
                if not self._running:
                    return
                time.sleep(0.5)

    def refresh(self) -> dict[str, Any]:
        """L'unica funzione che tocca la rete. Mai da un handler HTTP."""
        if not self.url:
            return self.status()
        try:
            data = http_get_json(self.url + "/status", self.cfg.proxy, timeout=3.0)
            allow = {"connected", "balance", "currency", "payout", "asset",
                     "mode", "account_type"}
            fields = ({k: v for k, v in data.items() if k in allow}
                      if isinstance(data, dict) else {})
            with self._lock:
                self.last_status = fields
                self.last_error = None
                self.last_fetch_ts = now_ms()
                self.fetches += 1
        except Exception as exc:  # noqa: BLE001
            with self._lock:
                self.last_error = f"{type(exc).__name__}: {exc}"
    ...
    def status(self) -> dict[str, Any]:
        """Solo memoria: non tocca la rete, quindi non blocca mai chi legge."""
        with self._lock:
            fields = dict(self.last_status)
            error = self.last_error
            fetched = self.last_fetch_ts
            fetches = self.fetches
        base = {
            "configured": bool(self.url), "connected": False,
            "bridge_url": self.url or None,
            "balance": None, "currency": self.cfg.wallet_currency,
            "payout": self.cfg.payout, "asset": self.cfg.symbol,
            "stake": self.cfg.stake_amount,
            # Questo programma NON invia ordini: non esiste al suo interno una
            # chiamata capace di piazzarne uno, e una verifica del selftest
            # legge il sorgente per garantirlo. Il bridge e' di sola lettura.
            "execution_supported": False,
            "orders_sent": 0,
            "note": ("bridge esterno di SOLA LETTURA: legge stato, saldo e "
                     "payout. Nessun ordine viene mai inviato da questo file."),
        }
        base.update(fields)
        base["last_error"] = error
        base["last_fetch_ts"] = fetched or None
        base["age_s"] = (round((now_ms() - fetched) / 1000.0, 1)
                         if fetched else None)
        base["fetches"] = fetches
        base["stale"] = bool(
            self.url and (not fetched
                          or now_ms() - fetched > self.cfg.pocket_bridge_poll_s * 3000)
        )
        return base
...
class Engine:
    ...
    def _external_features(self) -> dict[str, Any]:
        out: dict[str, Any] = {}
        fn = getattr(self.feed, "context_features", None)
        if callable(fn):
            out.update(fn() or {})
    ...
    def health(self) -> dict[str, Any]:
        ...
        components = {
            ...
            "news": {"status": ("UP" if self.news.status().get("news_available")
                                  else "DISABLED"), "detail": self.news.status()},
            "pocket": _bridge_health(self.pocket.status()),
        }
        overall = ("HEALTHY"
...
def cmd_check(cfg: Config, args) -> int:
    ...
    if cfg.source == "twelvedata":
        feed = TwelveDataFxFeed(cfg)
        rest_url = "https://api.twelvedata.com/time"
        ws_url = feed.ws_url
...
def cmd_selftest(cfg: Config, args) -> int:
    ...
    # ------------------------------------------------ EUR/USD: feed FX
    def t_fx_feed() -> str:
        """Il feed FX: normalizzazione, parsing, contesto, e cosa NON inventa."""
        c = Config(source="twelvedata", db_path=":memory:", http_port=0,
                   quiet=True)
        feed = TwelveDataFxFeed(c)

        for raw, want in (("EURUSD", "EUR/USD"), ("eur/usd", "EUR/USD"),
                          ("EUR-USD", "EUR/USD"), ("gbp_jpy", "GBP/JPY")):
            assert feed._norm(raw) == want, f"{raw} -> {feed._norm(raw)}"
        assert feed.symbols[0] == "EUR/USD", feed.symbols[:3]

        # Senza chiave non deve esplodere: deve dire cosa manca e restare vuoto.
        assert feed.poll(0.01) == [], "poll senza chiave deve essere vuoto"
        assert feed.last_error and "TWELVE_DATA_API_KEY" in feed.last_error, \
            feed.last_error

        # Il WebSocket dei prezzi non manda bid/ask ne' volumi: il feed lo
        # DICHIARA, cosi' i cancelli di spread e di flusso si spengono invece
        # di bocciare ogni finestra per un dato che non arrivera' mai.
        assert not feed.has_real_spread and not feed.supports_trades
        assert not feed.supports_depth

        # Contesto su un paniere vuoto: nessuna forza inventata.
        empty = feed.context_features()
        assert empty["fxctx_available"] == 0.0, empty["fxctx_available"]
        assert empty["fxctx_breadth"] == 0.0, empty["fxctx_breadth"]

        # Con storia vera: EUR sale contro USD, e la forza relativa lo riflette.
        base = now_ms()
        for sym, px, slope in (("EUR/USD", 1.0850, +1.0), ("GBP/USD", 1.2700, +1.0),
                               ("USD/JPY", 150.00, -1.0), ("USD/CHF", 0.8800, -1.0)):
            q = feed._series.setdefault(sym, deque(maxlen=1200))
            for i in range(70):
                q.append((base - 65_000 + i * 1000, px * (1 + slope * i * 2e-5)))
            feed._latest[sym] = (base, px * (1 + slope * 70 * 2e-5))
        ctx = feed.context_features()
        assert ctx["fxctx_available"] == 1.0, ctx["fxctx_available"]
        assert ctx["fxctx_pairs_fresh"] == 4.0, ctx["fxctx_pairs_fresh"]
        eur = ctx["fxctx_eur_strength_60s"]
        usd = ctx["fxctx_usd_strength_60s"]
        assert eur > 0 > usd or eur > usd, f"EUR {eur} contro USD {usd}"
        rel = ctx["fxctx_eurusd_relative_strength_60s"]
        assert rel > 0, f"forza relativa {rel} con EUR in salita contro USD"
        # Le coppie non aggiornate restano None: mai zero, che sarebbe un
        # movimento nullo inventato.
        assert ctx.get("fxctx_aud_usd_r60_bps") is None, "coppia muta non e' 0"
        return (f"{len(feed.symbols)} cross, forza EUR {eur:+.1f} vs USD "
                f"{usd:+.1f}, relativa {rel:+.1f}bps, coppie mute a None")

    # -------------------------------------- EUR/USD: news, bridge, prezzo
    def t_eurusd_portal() -> str:
        """News come cancello, bridge non bloccante, prezzi leggibili."""
        # 1. Il rischio evento alto deve BLOCCARE, non abbassare un punteggio.
        c = Config(db_path=":memory:", http_port=0, quiet=True,
                   min_warmup_s=0.0, news_enabled=True)
        news = MacroNewsScanner(c)
        snap = news.snapshot()
        assert snap["news_available"] == 0.0, snap
        assert snap["news_sentiment"] is None, "sentiment inventato senza dati"

        # 2. Il bridge NON deve toccare la rete quando lo si interroga.
        bridge = PocketBridge(Config(db_path=":memory:",
        ...
        assert elapsed < 0.5, (
            f"50 letture in {elapsed:.2f}s: status() sta ancora facendo rete, "
            "e finirebbe dentro le risposte HTTP")
        assert st["configured"] and st["age_s"] is None and st["stale"], st
        assert st["execution_supported"] is False and st["orders_sent"] == 0
        assert "live_execution" not in st, \
            "non deve esistere una bandiera che promette esecuzione"

        # 3. Il simulatore deve simulare LO STRUMENTO configurato.
        fx = SyntheticFeed(Config(source="sim", tick_size=1e-5, sim_seed=5))
        assert 0.5 < fx.price < 2.0, f"prezzo FX simulato {fx.price}"
        crypto = SyntheticFeed(Config(source="sim", tick_size=0.01, sim_seed=5))
        assert crypto.price > 1000, f"prezzo cripto simulato {crypto.price}"
        p0 = fx.price
        for _ in range(600):
            fx._step(0.1)
        move = abs(fx.price - p0) / p0 * 10_000.0
        assert move < 40.0, (
            f"il cambio simulato si e' mosso di {move:.0f}bps in 60s: e' "
            "volatilita' da criptovaluta, non da forex")

        # 4. I decimali di prezzo si ricavano dal tick, e la dashboard li usa.
        assert 'PRICE_DECIMALS = 2' in DASHBOARD_HTML
        assert 'function px(' in DASHBOARD_HTML
        assert 'priceDecimals(cfg.tick_size)' in DASHBOARD_HTML, \
            "il front end deve leggere il tick dalla configurazione"
        assert '"$"+num(sig.entry_price)' not in DASHBOARD_HTML, \
            "prezzi ancora formattati a due decimali con il simbolo del dollaro"
        # I tre pannelli nuovi devono esistere davvero.
        for panel in ("fx-body", "news-body", "br-body", "renderFx",
                      "renderNews", "renderBridge"):
            assert panel in DASHBOARD_HTML, f"manca {panel} nel front end"

        # 5. E la garanzia che conta: nessun percorso d'ordine nel file.
        forbidden = ("place" + "_order(", "create" + "_order(",
                     "send" + "_order(", "execute" + "_order(",
                     ".buy" + "(", ".sell" + "(")
        try:
        ...
        if source:
            ...
            assert not found, f"percorso d'ordine nel file: {found}"
        return (f"news mute senza dati, 50 letture del bridge in {elapsed*1000:.0f}ms "
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description=f"AURUM ENGINE {VERSION} - motore completo, un file solo, "
                    f"binarie PAPER a 60 secondi. Default: EUR/USD con contesto "
                    f"cross-FX + news, portafoglio da 500.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""esempi:
  %(prog)s check                          il venue e' raggiungibile?
  %(prog)s run --payout 0.8               EUR/USD CALL/PUT, scadenza 60s
                                          Twelve Data + news, dashboard :8002
...
""")
    ...
    p.add_argument("--source", choices=["twelvedata", "binance", "coinbase", "sim", "csv"])
    p.add_argument("--symbol")
    p.add_argument("--twelve-key", help="chiave Twelve Data (meglio TWELVE_DATA_API_KEY)")
    p.add_argument("--alpha-key", help="chiave Alpha Vantage (meglio ALPHAVANTAGE_API_KEY)")
    p.add_argument("--fx-context", help="coppie contesto separate da virgola")
    p.add_argument("--no-news", action="store_true", help="disattiva news macro/sentiment")
    p.add_argument("--pocket-bridge", help="URL bridge esterno/supportato per stato conto Pocket")
...
def apply_args(cfg: Config, args) -> Config:
    ...
    if getattr(args, "twelve_key", None):
        cfg.twelve_data_api_key = args.twelve_key
    if getattr(args, "alpha_key", None):
        cfg.alpha_vantage_api_key = args.alpha_key
    if getattr(args, "fx_context", None):
        cfg.fx_context_symbols = args.fx_context
