...
lato motore, tutto registrato su database e riaddestrato da solo.
    ...
    python3 aurum_engine.py burst                    # BURST solo analisi offline
...
Questa versione include anche un laboratorio di scoperta strategie M60: ricerca
simbolica su una grammatica chiusa di regole interpretabili, correzione FDR per
test multipli, holdout cronologico fresco e libreria di specialisti che puo'
affiancare (mai sostituire senza validazione) il modello principale. Restano
fuori i modelli ad alberi esterni (xgboost/lightgbm) e il frontend Next.js.
"""
...
VERSION = "2.0.0-PORTALE"
MODEL_VALIDATION_POLICY = "m60_research_lab_v3"
...
class Config:
    ...
    max_zero_move_fraction: float = 0.90
    ...
    min_expected_move_ticks: float = 0.5
    min_l1_notional: float = 500.0

    # -------------------------------------------------------------- segnali
    # OPERATIVITA': il motore live e' un prodotto da UN MINUTO.
    # BURST-15 resta disponibile solo come laboratorio/replay: le sue misure
    # microstrutturali (5s/10s) entrano come FEATURE del modello a +60s, non
    # aprono piu' operazioni a 5 secondi.
    strategy: str = "ensemble"       # live: ensemble 60s; burst15 solo offline
    ...
    # Due operazioni da 60s possono sovrapporsi. Con 1, il primo segnale
    # bloccava per un minuto intero ogni occasione successiva, rendendo il
    # cooldown da 15s praticamente inutile e imponendo un tetto artificiale.
    ...
    # Alla scadenza M60 si accetta solo un tick osservato vicino alla
    # scadenza. Se il feed resta fermo, il trade viene ANNULLATO invece
    # di inventare WIN/LOSS usando un prezzo vecchio.
    settlement_max_wait_ms: int = 2000
    ...
    # ---------------------------------------- microstruttura BURST -> target 60s
    # Queste soglie NON definiscono un trade a 5s. Producono feature che
    # descrivono l'impulso recente e che il modello a 60s puo' usare o ignorare.
    burst60_n5_min: int = 40
    burst60_r10_min_bps: float = 0.5
    burst60_soft_cap_bps: float = 2.0
    ...
    # Il dataset ML viene campionato a 1 Hz: 5000 righe sono ~83 minuti,
    # non 8 minuti di copie quasi identiche a 10 Hz.
    ml_min_samples: int = 5000
    ml_sample_interval_ms: int = 1000
    # Il verdetto OOS usa osservazioni distanziate almeno un intero orizzonte:
    # niente 600 etichette sovrapposte spacciate per 600 prove indipendenti.
    ml_oos_spacing_ms: int = 60000
    ml_min_independent_oos: int = 120
    ml_min_duration_minutes: float = 120.0
    ml_min_excess_vs_baseline: float = 0.02
    # Apprendimento continuo: non ristudiare lo stesso identico dataset. Un
    # giro vero richiede almeno 30 minuti di dati NUOVI dall'ultimo studio.
    ml_min_new_study_minutes: float = 30.0
    # Se esiste gia' un modello attivo, il nuovo e' un CHALLENGER: prima di
    # sostituirlo deve avere almeno un'ora di mercato mai vista dal champion.
    # I primi 30m possono entrare nel training del challenger; gli ultimi 30m
    # restano completamente freschi e servono solo per il duello.
    ml_challenger_fresh_minutes: float = 60.0
    ml_challenger_eval_minutes: float = 30.0
    ml_challenger_min_oos: int = 20
    ml_challenger_min_improvement: float = 0.01
    ml_embargo_s: float = 30.0
    ml_max_rows: int = 60000
    ml_epochs: int = 40
    ml_learning_rate: float = 0.08
    ml_l2: float = 1e-4
    # Margine minimo sopra il break-even economico del payout, quando noto.
    signal_ev_margin: float = 0.01

    # --------------------------------------------- laboratorio strategie M60
    # La ricerca NON genera Python e non modifica il motore da sola. Costruisce
    # regole matematiche interpretabili da una grammatica chiusa, le valida su
    # blocchi cronologici mai usati per crearle e promuove solo specialisti
    # sopravvissuti a correzione per test multipli + holdout fresco.
    research_enabled: bool = True
    research_top_features: int = 18
    research_pair_features: int = 7
    research_max_candidates: int = 128
    research_min_validation_signals: int = 12
    research_min_holdout_signals: int = 12
    research_fdr_alpha: float = 0.05
    research_holdout_p_max: float = 0.10
    research_min_margin_over_be: float = 0.015
    research_max_live_experts: int = 5
    research_live_weight_cap: float = 0.18

    # Adattamento online degli agenti: stima bayesiana regolarizzata + pesi
    # moltiplicativi. Il prior impedisce che 3 vittorie rendano un agente
    # improvvisamente dominante; il decadimento privilegia il regime recente.
    agent_prior_strength: float = 16.0
    agent_recency_decay: float = 0.985
    agent_mw_eta: float = 0.80
    agent_min_effective_samples: float = 12.0

    # Gli agenti euristici producono forza direzionale, non probabilita'
    # calibrate. Restringiamo quindi la confidence verso il 50%.
    # Un modello ML realmente calibrato non viene ristretto.
    heuristic_confidence_shrink: float = 0.65

    # -------------------------------------------------- POCKET OPTION (lettura)
    #: Il portale legge il broker per RISPONDERE A UNA DOMANDA: il segnale che
    #: Aurum ha appena emesso sarebbe eseguibile la' dove poi si opera davvero?
    #: Se i due prezzi divergono, se il feed e' vecchio, se il broker risponde
    #: lento, quel segnale non e' spendibile - e saperlo vale piu' di un
    #: segnale in piu'.
    #:
    #: NESSUN ORDINE VIENE MAI INVIATO. Non esiste in questo file una chiamata
    #: di acquisto, vendita o piazzamento: il collegamento e' di sola lettura
    #: (prezzo, saldo, stato della connessione).
    pocket_enabled: bool = False
    pocket_auth_file: str = ""
    pocket_asset: str = "BTCUSD"
    pocket_demo: bool = True
    pocket_poll_s: float = 0.75
    #: Soglie del filtro di eseguibilita'.
    pocket_max_diff_bps: float = 5.0
    pocket_max_query_ms: float = 750.0
    pocket_max_feed_age_ms: float = 750.0
    pocket_max_signal_age_ms: float = 1500.0
    #: Puntata e durata che si USEREBBERO sul broker. Sono un'annotazione, non
    #: un ordine: servono a rendere confrontabile il registro con il broker.
    pocket_stake: float = 2.0
    pocket_duration_s: int = 60

    # ------------------------------------------------------- filtro del rumore
    #: Quante valutazioni consecutive devono indicare la STESSA direzione prima
    #: di emettere. Una finestra da 100 ms che gira da sola e' rumore: chiedere
    #: che la direzione regga per qualche finestra e' il filtro piu' economico
    #: che esista contro l'artefatto singolo.
    signal_persistence_windows: int = 3
    #: Il movimento atteso sull'orizzonte deve superare il rumore di fondo
    #: (spread + un tick di quantizzazione) di questo fattore. E' una soglia
    #: RELATIVA al mercato, non un numero assoluto di tick.
    min_move_over_noise: float = 1.5
    ...
    def __post_init__(self) -> None:
        ...
        if self.strategy == "ensemble":
            # Il prodotto live e' SEMPRE un minuto. Un vecchio --horizon o una
            # variabile d'ambiente non devono trasformarlo silenziosamente.
            self.horizon_s = 60.0
            derived.add("horizon_s")
        elif self.horizon_s <= 0 or "horizon_s" in derived:
            self.horizon_s = self.HORIZON_BY_STRATEGY.get(self.strategy, 5.0)
            derived.add("horizon_s")
        ...
        # `ml_min_samples` resta una soglia esplicita. La purga temporale non
        # deve gonfiarla: walk-forward e calibrazione ora rimuovono il gap dalla
        # CODA del training, lasciando intatto il blocco futuro di test/calibrazione.
        # In questo modo 5000 righe a 10 Hz sono realmente utilizzabili anche
        # con un orizzonte da 60s, invece di trasformarsi in ~12500 righe e
        # rimandare il primo studio di oltre venti minuti.
        self.ml_min_samples = max(300, int(self.ml_min_samples))
        self._derived = derived
    ...
    def effective_min_confidence(self) -> float:
        """Soglia direzionale minima, anche economicamente sensata.

        Con payout noto non basta superare 50%: per payout 0.8 il pareggio e'
        55.56%. Aggiungiamo un piccolo margine per non accettare segnali che,
        anche se calibrati perfettamente, avrebbero EV nullo o negativo.
        """
        base = max(self.min_confidence, 0.5 + self.min_edge)
        if self.payout is not None and self.payout > 0:
            base = max(base, breakeven_win_rate(self.payout) + self.signal_ev_margin)
...
def binomial_upper_p_value(successes: int, n: int, p0: float) -> float:
    """P(H0: p<=p0) per un risultato osservato migliore di p0.

    Approssimazione normale con correzione di continuita'. Per l'edge economico
    p0 e' il break-even del payout, non 0.5.
    """
...
-- Stato persistente dell'apprendimento. Serve a non ripetere lo stesso studio
-- dopo un restart e a sapere quanta informazione nuova e' arrivata davvero.
CREATE TABLE IF NOT EXISTS learning_state (
    key TEXT PRIMARY KEY, ts INTEGER NOT NULL, value TEXT
);

-- Verifica di eseguibilita' presso il broker. Una riga per segnale: il verdetto
-- e il motivo, con i numeri su cui e' stato preso. NON e' un registro di
-- ordini: nessun ordine viene mai inviato.
CREATE TABLE IF NOT EXISTS pocket_checks (
    signal_id TEXT PRIMARY KEY, ts INTEGER NOT NULL, direction TEXT,
    confidence REAL, regime TEXT, asset TEXT,
    aurum_price REAL, pocket_price REAL, diff_bps REAL,
    feed_age_ms REAL, query_ms REAL, signal_age_ms REAL,
    verdict TEXT, reason TEXT, stake REAL, duration_s INTEGER,
    result TEXT, is_synthetic INTEGER
);
CREATE INDEX IF NOT EXISTS ix_pocket_ts ON pocket_checks(ts);
"""
...
class Store:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self._pocket: "queue.Queue[dict]" = queue.Queue(maxsize=20_000)
    ...
    POCKET_COLUMNS: tuple[str, ...] = (
        "signal_id", "ts", "direction", "confidence", "regime", "asset",
        "aurum_price", "pocket_price", "diff_bps", "feed_age_ms", "query_ms",
        "signal_age_ms", "verdict", "reason", "stake", "duration_s", "result",
        "is_synthetic",
    )
    ...
    _COLUMN_TYPES: dict[str, str] = {
        "verdict": "TEXT", "reason": "TEXT", "asset": "TEXT", "result": "TEXT",
        "stake": "REAL", "duration_s": "INTEGER", "direction": "TEXT",
    ...
    def _migrate(self) -> list[str]:
        ...
        wanted["pocket_checks"] = self.POCKET_COLUMNS
        added: list[str] = []
    ...
    def upsert_pocket_check(self, row: dict[str, Any]) -> None:
        try:
            self._pocket.put_nowait(row)
        except queue.Full:
            self.stats["dropped"] += 1

    def set_pocket_result(self, signal_id: str, result: str | None) -> None:
        """L'esito arriva dopo il verdetto: la riga si aggiorna, non si duplica."""
        self.upsert_pocket_check({"signal_id": signal_id, "ts": now_ms(),
                                  "result": result})

    def pocket_checks(self, limit: int | None = None) -> list[dict]:
        conn = self.reader()
        try:
            sql = "SELECT * FROM pocket_checks ORDER BY ts DESC"
            if limit:
    ...
    def flush(self) -> None:
        ...
        while True:
            try:
                checks.append(self._pocket.get_nowait())
            except queue.Empty:
                break

        if not batches and not papers and not checks:
            return
        ...
        with self._write_lock:
            ...
            for row in checks:
                keys = list(row)
                sql = (
                    f"INSERT INTO pocket_checks ({','.join(keys)}) "
                    f"VALUES ({','.join('?' * len(keys))}) "
                    f"ON CONFLICT(signal_id) DO UPDATE SET "
                ...
                try:
                    ...
                    failures.append(f"pocket_checks: {exc}")
                    self.stats["dropped"] += 1
            self._conn.commit()
            if failures:
    ...
    def feature_rows(self, include_synthetic: bool = True) -> int:
        return int(self.feature_stats(include_synthetic)["rows"])

    def feature_stats(self, include_synthetic: bool = True) -> dict[str, Any]:
        """Conteggio e durata delle feature reali disponibili per lo studio."""
        conn = self.reader()
        try:
            where = "" if include_synthetic else " WHERE is_synthetic = 0"
            r = conn.execute(
                f"SELECT COUNT(*) AS n, MIN(ts) AS lo, MAX(ts) AS hi FROM features{where}"
            ).fetchone()
            n = int(r["n"] or 0)
            lo, hi = r["lo"], r["hi"]
            span_ms = max(0, int(hi - lo)) if lo is not None and hi is not None else 0
            return {"rows": n, "min_ts": lo, "max_ts": hi, "span_ms": span_ms}
        finally:
            conn.close()
    ...
    def get_learning_state(self, key: str, default: Any = None) -> Any:
        conn = self.reader()
        try:
            row = conn.execute(
    ...
    def set_learning_state(self, key: str, value: Any) -> None:
        ...
        with self._write_lock:
            self._conn.execute(
                "INSERT INTO learning_state(key, ts, value) VALUES(?,?,?) "
    ...
    def active_model_artifact(self, horizon_s: float | None = None) -> dict | None:
        """Carica solo un modello attivo compatibile con l'orizzonte richiesto."""
        conn = self.reader()
        try:
            if horizon_s is None:
                ...
                row = conn.execute(
                    "SELECT artifact FROM model_versions WHERE is_active = 1 "
                    "AND ABS(horizon_s - ?) < 0.001 ORDER BY ts DESC LIMIT 1",
                    (float(horizon_s),),
                ).fetchone()
...
class FeatureEngine:
    ...
    def compute(self, ts: int | None = None) -> dict[str, Any] | None:
        ...
        # ------------------------------------ BURST come contesto per +60 secondi
        # Nessuna di queste feature apre un trade a 5s. Riassumono cio' che e'
        # successo negli ultimi 5/10 secondi per aiutare il target a UN MINUTO.
        n5 = float(f.get("trade_count_5s") or 0.0)
        r10 = float(f.get("return_10000ms") or 0.0)
        vi5 = float(f.get("volume_imbalance_5s") or 0.0)
        abs_r10 = abs(r10)
        sign_r10 = 1.0 if r10 > 0 else -1.0 if r10 < 0 else 0.0
        activity_ratio = min(4.0, n5 / max(float(cfg.burst60_n5_min), 1.0))
        ofi_agree = (1.0 if r10 * vi5 > 0 else -1.0 if r10 * vi5 < 0 else 0.0)
        active = 1.0 if (n5 >= cfg.burst60_n5_min and
                         abs_r10 >= cfg.burst60_r10_min_bps) else 0.0
        # I dati storici mostrano che un impulso moderato puo' continuare, ma
        # gli strappi estremi sono meno affidabili a +60s. Non imponiamo una
        # reversal: forniamo al modello DUE feature distinte, cosi' e' il
        # walk-forward a decidere il loro segno.
        taper = 1.0 / (1.0 + max(0.0, abs_r10 - cfg.burst60_soft_cap_bps))
        strength = math.tanh(abs_r10 / max(cfg.burst60_r10_min_bps, 0.05))
        f["burst60_active"] = active
        f["burst60_direction"] = sign_r10
        f["burst60_activity_ratio"] = activity_ratio
        f["burst60_r10_abs_bps"] = abs_r10
        f["burst60_ofi_agreement"] = ofi_agree
        f["burst60_continuation_score"] = (
            sign_r10 * active * activity_ratio * strength * taper
            * (1.0 if ofi_agree > 0 else 0.25 if ofi_agree == 0 else -0.25)
        )
        f["burst60_exhaustion_score"] = (
            sign_r10 * max(0.0, abs_r10 - cfg.burst60_soft_cap_bps)
            * max(activity_ratio, 0.25)
        )
        ...
        #: Serve a misurare il rumore di fondo in punti base: senza la
        #: dimensione del tick, "spread + un tick" non e' calcolabile.
...
class VolatilityAgent(Agent):
    """Non sceglie un lato: verifica se il target a 60s e' negoziabile."""
    ...
    def _evaluate(self, ctx: Ctx) -> AgentOutput:
        ...
        sigma_h = ctx.features.get("sigma_horizon_bps")
        if v5 is None or v30 is None:
            return self.abstain("volatilita' non ancora stimabile")
        spread = ctx.f("spread_bps", 0.0) or 0.0
        mid = ctx.f("mid", 0.0) or 0.0
        tick_bps = (self.setting("tick_size", 0.01) / mid * 10_000.0
                    if mid > 0 else 1e-6)
        # BUG 1.1: max(spread, 0.05) trasformava uno spread BTC di ~0.0016bps
        # in 0.05bps, oltre 30x piu' grande, bloccando il 60s quasi sempre.
        effective_spread = max(spread, tick_bps, 1e-6)
        # Per un target a 60s il movimento rilevante e' sigma(60s), non la sola
        # volatilita' degli ultimi 5 secondi.
        move_bps = sigma_h if sigma_h is not None and sigma_h > 0 else v5
        edge_ratio = move_bps / effective_spread
        min_ticks = self.setting("min_expected_move_ticks", 0.5)
        max_flat = self.setting("max_zero_move_fraction", 0.90)
        ...
        conf = min(0.95, edge_ratio / 4.0) if tradable else 0.0
        reason = (f"sigma{self.setting('horizon_s', 60):g}s {move_bps:.3f}bps "
                  f"vs spread effettivo {effective_spread:.4f}bps (x{edge_ratio:.1f})")
        if not moves_enough:
        ...
        out = self.emit(0.0, conf, reason,
                        ["sigma_horizon_bps", "expected_move_ticks",
...
class DecisionEngine:
    ...
    def __init__(self, cfg: Config, model: "Model | None" = None,
        ...
        #: Stato della persistenza direzionale fra una finestra e l'altra.
        self._last_lean: str = NO_TRADE
        self._lean_streak: int = 0
        self._lean_ts: int = 0
    ...
    def decide(self, vector: dict, market: dict, health: dict) -> Decision:
        ...
        # ------------------------------------------------ rumore di fondo
        # `min_expected_move_ticks` e' una soglia ASSOLUTA: due tick valgono
        # tanto su un mercato calmo quanto su uno agitato, e su un altro
        # simbolo non vogliono dire niente. Questa e' relativa: il movimento
        # atteso sull'orizzonte deve battere il costo implicito di un giro -
        # lo spread piu' un tick di quantizzazione - di un margine dichiarato.
        # Sotto quella soglia la direzione non e' sbagliata: e' indistinguibile
        # dal rumore, che e' una cosa diversa e piu' onesta da dire.
        noise = self._noise_floor_bps(f)
        sigma_h = f.get("sigma_horizon_bps")
        if noise is not None and sigma_h:
            if sigma_h < noise * cfg.min_move_over_noise:
                reasons.append(
                    f"movimento atteso {sigma_h:.2f}bps sotto il rumore di fondo "
                    f"{noise:.2f}bps x{cfg.min_move_over_noise:g}")
        ...
        if self.model is not None and self.model.ready:
            if abs(float(self.model.horizon_s) - float(cfg.horizon_s)) > 0.001:
                # Mai piu' un modello h5 dentro una decisione h60.
                model_note = (f"modello {self.model.model_id} ignorato: orizzonte "
                              f"{self.model.horizon_s:g}s != {cfg.horizon_s:g}s")
            else:
                out = self.model.predict(f)
                if out is None:
                    model_note = "modello non applicabile a queste feature"
                elif out.get("out_of_distribution"):
                    model_note = "modello fuori distribuzione: " + str(out.get("reason"))
                else:
                    model_prob = float(out["prob_up"])
                    model_id = out.get("model_id")
                    calibrated = bool(out.get("calibrated"))
                    # Un modello 60s promosso su OOS indipendente deve contare
                    # piu' dei pesi euristici; l'ensemble resta un correttivo.
                    w = 0.65 if calibrated else 0.30
                    directional = p_up / max(p_up + p_down, 1e-9)
                    blended = (1 - w) * directional + w * model_prob
                    mass = p_up + p_down
                    p_up, p_down = blended * mass, (1 - blended) * mass

        strategy_research_live = {"active": 0, "prob_up": None, "weight": 0.0, "votes": []}
        if self.model is not None and self.model.ready and cfg.research_enabled:
            try:
                strategy_research_live = strategy_library_prediction(self.model, f, cfg)
                if strategy_research_live.get("active") and strategy_research_live.get("prob_up") is not None:
                    rw = float(strategy_research_live.get("weight") or 0.0)
                    directional0 = p_up / max(p_up + p_down, 1e-9)
                    blended = ((1.0-rw)*directional0 +
                               rw*float(strategy_research_live["prob_up"]))
                    mass = p_up + p_down
                    p_up, p_down = blended*mass, (1.0-blended)*mass
            except Exception as exc:
                strategy_research_live = {"active": 0, "prob_up": None, "weight": 0.0,
                                          "votes": [], "error": str(exc)}

        directional_raw = p_up / max(p_up + p_down, 1e-9)

        # L'ensemble euristico non e' una probabilita' calibrata.
        # Finche' non esiste un vero modello ML calibrato, restringiamo
        # la probabilita' verso 50% per evitare confidence artificiali.
        shrink = (
            1.0
            if calibrated
            else max(
                ...
                min(
                    1.0,
                    float(cfg.heuristic_confidence_shrink)
                )
            )
        )
        ...
        p_up = directional * mass
        p_down = (1.0 - directional) * mass
        ...
        # La pendenza va aggiornata a OGNI finestra, anche quando qualche
        # cancello blocca: altrimenti la serie conterebbe solo le finestre gia'
        # pulite e "tre di fila" non vorrebbe piu' dire tre di fila.
        lean_now = UP if directional > 0.5 else DOWN
        steady, streak = self._persistent(
            lean_now if mass >= cfg.min_agreement else NO_TRADE, ts
        )
        if not steady:
            reasons.append(
                f"direzione non ancora stabile: {streak} finestre di fila su "
                f"{max(1, int(cfg.signal_persistence_windows))} richieste")
        ...
        return Decision(
            ...
            detail={
                ...
                "confidence_raw": round(
                    max(directional_raw, 1.0-directional_raw), 4
                ),
                "confidence_shrink": round(shrink, 4),
                "sigma_horizon_bps": f.get("sigma_horizon_bps"),
                "noise_floor_bps": (round(noise, 3) if noise is not None else None),
                "lean_streak": streak,
                "persistence_required": max(1, int(cfg.signal_persistence_windows)),
                "model_note": model_note,
                "strategy_research_live": strategy_research_live,
                "burst60": {
                    "active": f.get("burst60_active"),
                    "r10_abs_bps": f.get("burst60_r10_abs_bps"),
                    "ofi_agreement": f.get("burst60_ofi_agreement"),
                    "continuation_score": f.get("burst60_continuation_score"),
                    "exhaustion_score": f.get("burst60_exhaustion_score"),
                },
                "thresholds": {
    ...
    def _aggregate(self, outputs: list[AgentOutput], regime: str):
        ...
        if self.performance is not None:
            try:
                try:
                    hit_rates = self.performance(regime) or {}
                except TypeError:
        ...
        for agent in self.agents:
            ...
            if hr is not None:
                # Multiplicative-weights regolarizzato: un agente sopra 50%
                # cresce esponenzialmente ma con clip; sotto 50% perde voce.
                advantage = (float(hr) - 0.5) * 2.0
                w *= max(0.55, min(1.55, math.exp(cfg.agent_mw_eta * advantage)))
    ...
    @staticmethod
    def _noise_floor_bps(f: dict) -> float | None:
        """Il costo implicito di un giro, in punti base.

        Spread piu' un tick: e' cio' che il prezzo deve muoversi prima che un
        movimento sia distinguibile da un aggiustamento del book.
        """
        spread = f.get("spread_bps")
        mid, tick = f.get("mid"), f.get("tick_size")
        if spread is None:
            return None
        quant = (tick / mid * 10_000.0) if (mid and tick) else 0.0
        return max(0.01, float(spread) + quant)

    def _persistent(self, direction: str, ts: int) -> tuple[bool, int]:
        """Da quante valutazioni consecutive la direzione e' questa.

        Il motore valuta dieci volte al secondo. Una singola finestra che gira
        e poi torna indietro e' rumore, non un cambio di opinione: chiedere che
        la direzione regga qualche finestra elimina l'artefatto singolo senza
        toccare nessuna soglia di confidenza.
        """
        if direction != self._last_lean:
            self._last_lean = direction
            self._lean_streak = 1
        else:
            self._lean_streak += 1
        self._lean_ts = ts
        needed = max(1, int(self.cfg.signal_persistence_windows))
        return (self._lean_streak >= needed, self._lean_streak)
...
class LiveSignal:
    ...
    #: Verdetto di eseguibilita' presso il broker, preso all'istante del
    #: segnale. None finche' il collegamento e' spento.
...
class SignalEngine:
    ...
    def __init__(self, cfg: Config, market: MarketData, features: FeatureEngine,
        ...
        #: Il collegamento al broker e il suo giudizio di eseguibilita'. Vivono
        #: qui perche' il verdetto va preso NEL momento del segnale: farlo dopo,
        #: da un altro processo che interroga l'API, misurerebbe un altro
        #: istante e quindi un'altra cosa.
        self.pocket = PocketLink(cfg)
        self.pocket_gate = PocketGate(cfg, self.pocket, store)
    ...
    def _create(self, decision: Decision, vector: dict) -> LiveSignal:
        ...
        sig = LiveSignal(
            ...
            features={
                **vector.get("features", {}),
                "_agents": {o.agent: o.direction for o in decision.agents},
                "_agent_scores": {o.agent: round(o.score, 6) for o in decision.agents},
                "_agent_confidences": {
                    o.agent: round(o.confidence, 6) for o in decision.agents
                },
            },
        )
        ...
        # Il verdetto del broker si prende adesso, sullo stesso istante in cui
        # il segnale nasce: e' l'unico momento in cui la domanda "sarebbe
        # eseguibile?" ha una risposta vera.
        try:
            sig.pocket = self.pocket_gate.check(
                sig, self.market.snapshot(), self.market.health())
        except Exception as exc:  # noqa: BLE001 - il broker non ferma il motore
            sig.pocket = {"verdict": UNVERIFIED,
        ...
        for sid in list(self.active):
            ...
            if sig.status == WAITING:
                ...
                if ts >= sig.expires_at:
                    # Non usare un tick precedente alla scadenza.
                    # Attendiamo il primo tick a/oltre expiry ma solo entro
                    # una tolleranza breve. Se il feed e' fermo, il trade
                    # viene annullato e non contamina le statistiche.
                    tick = self.market.last_tick
                    max_wait = max(0, int(self.cfg.settlement_max_wait_ms))
                    tick_ts = int(tick.ts) if tick is not None else None

                    if (
                        tick_ts is not None
                        and tick_ts >= sig.expires_at
                        and tick_ts - sig.expires_at <= max_wait
                    ):
                        self._expire(sig, self._price(), tick_ts)

                    elif ts - sig.expires_at > max_wait:
                        self._cancel(
                            sig,
                            ts,
                            f"settlement non affidabile: nessun tick entro "
                            f"{max_wait}ms dalla scadenza",
                        )
    ...
    def _finish(self, sig: LiveSignal) -> None:
        ...
        # L'esito torna sulla riga di verifica: senza, non si potrebbe mai
        # rispondere alla domanda che conta - i segnali giudicati eseguibili
        # hanno vinto piu' spesso degli altri?
        self.pocket_gate.settle(sig.signal_id, sig.result)
    ...
    # ------------------------------------------------------------------ viste
    def agent_hit_rates(self, regime: str | None = None) -> dict[str, float]:
        """Affidabilita' online degli agenti con prior + decadimento temporale.

        Conta anche gli output con direction=NO_TRADE ma score non-zero:
        quei punteggi entrano comunque nell'ensemble e quindi devono ricevere
        credito oppure penalita'.

        Il contributo viene pesato per la confidence registrata. Sui trade
        vecchi, dove la confidence individuale non esiste ancora, usa |score|
        come proxy della forza con cui l'agente ha influenzato la decisione.
        """
        cfg = self.cfg
        ...
        def unpack_features(feats: dict[str, Any]):
            return (
                feats.get("_agents") or {},
                feats.get("_agent_scores") or {},
                feats.get("_agent_confidences") or {},
            )

        for sig in self.history[-500:]:
            if (
                sig.result not in (WIN, LOSS)
                or sig.entry_price is None
                or sig.expiry_price is None
            ):
                continue

            actual_up = sig.expiry_price > sig.entry_price
            agents, scores, confs = unpack_features(sig.features or {})

            records.append(
                (sig.regime, actual_up, agents, scores, confs)
            )

        if len(records) < 40 and self.store is not None:
            try:
                rows = self.store.paper_trades(
                    limit=500,
                ...
                for r in reversed(rows):
                    if (
                        r.get("result") not in (WIN, LOSS)
                        or r.get("entry_price") is None
                        or r.get("expiry_price") is None
                    ):
                        continue
                    ...
                    agents, scores, confs = unpack_features(feats)

                    records.append(
                        (
                            r.get("regime"),
                            float(r["expiry_price"]) > float(r["entry_price"]),
                            agents,
                            scores,
                            confs,
                        )
        ...
        def estimate(selected: list[Record]) -> dict[str, float]:
            tally: dict[str, list[float]] = {}
            total = len(selected)

            decay = max(
                0.90,
                min(0.9999, float(cfg.agent_recency_decay))
            )

            for pos, (_, actual_up, agents, scores, confs) in enumerate(selected):
                age = total - 1 - pos
                rw = decay ** age

                names = set(agents) | set(scores)
                ...
                for name in names:
                    ...
                    # Se l'agente non superava la propria soglia ma il suo
                    # score entrava comunque nell'ensemble, ricaviamo la
                    # direzione dal segno dello score.
                    if out not in (UP, DOWN):
                        if score == 0.0:
                            continue
                        out = UP if score > 0 else DOWN

                    try:
                        influence = float(confs.get(name))
                    except (TypeError, ValueError):
                        influence = abs(score)
                    ...
                    a = tally.setdefault(name, [0.0, 0.0])

                    a[0] += (
                        rw
                        * influence
                        * int((out == UP) == actual_up)
                    )
            ...
            prior = max(
                0.0,
                float(cfg.agent_prior_strength)
            )

            result = {}

            for name, (wins, eff_n) in tally.items():
                if eff_n < cfg.agent_min_effective_samples:
                    continue
        ...
        if regime:
            local = [r for r in records if r[0] == regime]
            local_est = estimate(local)
    ...
    * la puntata si fissa all'ingresso. Con la puntata in percentuale,
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cycle": self.cycle,
            "started_ts": self.cycle_start_ts,
...
def build_dataset(store: Store, cfg: Config, horizon_s: float,
    ...
    """Dataset causale per il target a +60s.

    Le feature arrivano solo dal passato; la label arriva dal primo tick vicino
    a ts+horizon. Le righe vengono campionate a 1Hz per non addestrare dieci
    copie quasi identiche dello stesso stato di mercato ogni secondo.
    """
    conn = store.reader()
    try:
        where = "" if include_synthetic else " WHERE is_synthetic = 0"
        frows = conn.execute(
            f"SELECT ts, mid, payload, data_quality, book_synced "
            f"FROM features{where} ORDER BY ts ASC"
        ).fetchall()
        trows = conn.execute(
    ...
    sample_ms = max(100, int(cfg.ml_sample_interval_ms))

    parsed: list[tuple[int, float, dict]] = []
    dropped_quality = dropped_book = dropped_warmup = dropped_dense = 0
    last_kept = -10**18
    for r in frows:
        ts = int(r["ts"])
        if ts - last_kept < sample_ms:
            dropped_dense += 1
            continue
        try:
            ...
            continue
        # Compatibilita' con i database 1.0/1.1: ricostruisci le nuove feature
        # BURST->60s dalle misure grezze gia' registrate, senza usare futuro.
        if "burst60_continuation_score" not in payload:
            n5 = float(payload.get("trade_count_5s") or 0.0)
            r10 = float(payload.get("return_10000ms") or 0.0)
            vi5 = float(payload.get("volume_imbalance_5s") or 0.0)
            abs_r10 = abs(r10)
            sign_r10 = 1.0 if r10 > 0 else -1.0 if r10 < 0 else 0.0
            activity_ratio = min(4.0, n5 / max(float(cfg.burst60_n5_min), 1.0))
            ofi_agree = (1.0 if r10 * vi5 > 0 else
                         -1.0 if r10 * vi5 < 0 else 0.0)
            active = 1.0 if (n5 >= cfg.burst60_n5_min and
                             abs_r10 >= cfg.burst60_r10_min_bps) else 0.0
            taper = 1.0 / (1.0 + max(0.0, abs_r10 - cfg.burst60_soft_cap_bps))
            strength = math.tanh(abs_r10 / max(cfg.burst60_r10_min_bps, 0.05))
            payload["burst60_active"] = active
            payload["burst60_direction"] = sign_r10
            payload["burst60_activity_ratio"] = activity_ratio
            payload["burst60_r10_abs_bps"] = abs_r10
            payload["burst60_ofi_agreement"] = ofi_agree
            payload["burst60_continuation_score"] = (
                sign_r10 * active * activity_ratio * strength * taper
                * (1.0 if ofi_agree > 0 else 0.25 if ofi_agree == 0 else -0.25)
            )
            payload["burst60_exhaustion_score"] = (
                sign_r10 * max(0.0, abs_r10 - cfg.burst60_soft_cap_bps)
                * max(activity_ratio, 0.25)
            )
        dq = r["data_quality"]
        if dq is not None and float(dq) < cfg.min_data_quality:
            dropped_quality += 1
            continue
        book_expected = bool(payload.get("book_expected", True))
        if book_expected and not bool(r["book_synced"]):
            dropped_book += 1
            continue
        span = payload.get("history_span_ms")
        if span is not None and float(span) < cfg.min_warmup_s * 1000.0:
            dropped_warmup += 1
            continue
        parsed.append((ts, r["mid"], payload))
        last_kept = ts

    if max_rows and len(parsed) > max_rows:
        # Memoria lunga + adattamento recente. Meta' del budget conserva una
        # vista uniforme del passato, meta' tiene CONTIGUA la coda piu' recente:
        # gli errori appena osservati non vengono diluiti da settimane di storia.
        recent_n = max(1, max_rows // 2)
        old_n = max_rows - recent_n
        recent = parsed[-recent_n:]
        older = parsed[:-recent_n]
        if old_n > 0 and older:
            step = len(older) / old_n
            old_sample = [older[min(len(older) - 1, int(i * step))]
                          for i in range(old_n)]
        else:
            old_sample = []
        parsed = old_sample + recent

    # Unione su tutto il dataset campionato: le feature nuove (es. BURST60)
    # non devono sparire solo perche' non erano presenti nelle prime righe.
    names: list[str] = sorted({
        k for _, _, payload in parsed
    ...
    for ts, mid, payload in parsed:
        ...
        if mid is None or mid <= 0 or exit_price is None:
            continue
        if exit_price == mid:
            ties += 1
            continue
        ...
        for name in names:
            ...
            if (isinstance(v, (int, float)) and not isinstance(v, bool)
                ...
                # NaN e' un MISSING esplicito: fit e predict lo imputano con la
                # stessa media di training. Prima train=0 e live=media divergevano.
    ...
    # Colonne senza almeno due valori FINITI distinti non portano informazione.
    keep: list[int] = []
    if rows:
        for i in range(len(names)):
            vals = {r[i] for r in rows[:5000] if math.isfinite(r[i])}
            if len(vals) > 1:
                keep.append(i)
    ...
    return Dataset(
        ...
        {"dropped_no_future_tick": dropped_no_future, "ties_dropped": ties,
         "raw_feature_rows": len(frows), "sampled_rows_scanned": len(parsed),
         "ml_sample_interval_ms": sample_ms, "dropped_dense": dropped_dense,
         "dropped_quality": dropped_quality, "dropped_book": dropped_book,
...
def _standardise(rows: list[list[float]]) -> tuple[list[float], list[float]]:
    ...
    for i in range(cols):
        vals = [float(r[i]) for r in rows
                if isinstance(r[i], (int, float)) and math.isfinite(float(r[i]))]
        if not vals:
            mean.append(0.0)
            std.append(1.0)
            continue
...
def fit_logistic(rows: list[list[float]], y: list[int], cfg: Config,
    ...
    """Logistica L2: semplice, causale e robusta ai missing."""
    ...
    def xval(row: list[float], i: int) -> float:
        raw = row[i]
        if not isinstance(raw, (int, float)) or not math.isfinite(float(raw)):
            raw = mean[i]
        x = (float(raw) - mean[i]) / std[i]
    ...
    for epoch in range(cfg.ml_epochs):
        ...
        for start in range(0, len(idx), batch):
            ...
            for j in chunk:
                row = rows[j]
                xs = [xval(row, i) for i in range(cols)]
                z = b + sum(w[i] * xs[i] for i in range(cols))
...
def walk_forward_folds(ts: list[int], n_splits: int, horizon_s: float,
    ...
    for k in range(1, n_splits + 1):
        # Il blocco di test resta interamente nel FUTURO. Il purge va applicato
        # alla coda del training: spostare in avanti `test_start` (come prima)
        # divorava il blocco di test quando gap > durata del blocco e produceva
        # zero fold proprio sugli orizzonti lunghi.
        test_start = block * k
        test_end = min(n, block * (k + 1))
        if test_start >= test_end:
            continue
        train_cutoff_ts = ts[test_start] - gap_ms
        train_end = bisect.bisect_right(ts, train_cutoff_ts, lo=0, hi=test_start)
...
def _spaced_indices(ts: list[int], start: int, end: int, spacing_ms: int) -> list[int]:
    out: list[int] = []
    last = -10**18
    for i in range(start, end):
        if ts[i] - last >= spacing_ms:
            out.append(i)
            last = ts[i]
    return out
...
def run_walk_forward(ds: Dataset, cfg: Config, n_splits: int) -> dict[str, Any]:
    """Walk-forward purged con DUE metriche.

    `dense` e' diagnostica. Il verdetto usa solo `out_of_sample`: righe di test
    distanziate almeno un intero orizzonte, quindi le label non si sovrappongono.
    """
    ...
    fold_reports = []
    dense_correct = dense_n = 0
    ind_correct = ind_n = 0
    baseline_correct = 0
    up_correct = up_n = down_correct = down_n = 0
    spacing_ms = max(int(ds.horizon_s * 1000), int(cfg.ml_oos_spacing_ms))
    threshold = breakeven_win_rate(cfg.payout) if cfg.payout is not None else 0.5

    for i, fold in enumerate(folds):
        tr0, tr1 = fold.train
        te0, te1 = fold.test
        if len(set(ds.y[tr0:tr1])) < 2:
            continue
        ...
        independent = _spaced_indices(ds.ts, te0, te1, spacing_ms)
        train_up = sum(ds.y[tr0:tr1])
        baseline_pred = 1 if train_up >= (tr1 - tr0 - train_up) else 0
        ic = bc = iu = iuc = idn = idc = 0
        for j in independent:
            truth = ds.y[j]
            pred = 1 if model.predict_row(ds.rows[j]) >= 0.5 else 0
            ic += int(pred == truth)
            bc += int(baseline_pred == truth)
            if truth == 1:
                iu += 1; iuc += int(pred == truth)
            else:
                idn += 1; idc += int(pred == truth)
        ind_correct += ic
        ind_n += len(independent)
        baseline_correct += bc
        up_n += iu; up_correct += iuc
        down_n += idn; down_correct += idc
        fold_reports.append({
            "fold": i, "train_rows": tr1 - tr0, "test_rows_dense": n,
            "test_rows_independent": len(independent),
            "accuracy_dense": round(correct / n, 4) if n else None,
            "accuracy_independent": round(ic / len(independent), 4) if independent else None,
            "baseline_accuracy_independent": round(bc / len(independent), 4) if independent else None,
            "purge_gap_ms": fold.purge_gap_ms,
        })

    if not fold_reports or not ind_n:
        return {"error": "nessuna osservazione OOS indipendente disponibile"}
    lo, hi = wilson_interval(ind_correct, ind_n)
    acc = ind_correct / ind_n
    baseline_acc = baseline_correct / ind_n
    p_be = binomial_upper_p_value(ind_correct, ind_n, threshold)
    bal_acc = None
    if up_n and down_n:
        bal_acc = 0.5 * (up_correct / up_n + down_correct / down_n)
    return {
        "folds": fold_reports,
        "oos_spacing_ms": spacing_ms,
        "out_of_sample_dense": {
            "n": dense_n, "correct": dense_correct,
            "accuracy": round(dense_correct / dense_n, 5) if dense_n else None,
        },
        "out_of_sample": {
            "n": ind_n, "correct": ind_correct, "accuracy": round(acc, 5),
            "accuracy_ci95": [round(lo, 5), round(hi, 5)],
            "p_value_vs_breakeven": round(p_be, 6),
            "breakeven_win_rate": round(threshold, 6),
            "baseline_accuracy": round(baseline_acc, 5),
            "excess_vs_baseline": round(acc - baseline_acc, 5),
            "balanced_accuracy": round(bal_acc, 5) if bal_acc is not None else None,
            "up": {"n": up_n, "accuracy": round(up_correct / up_n, 5) if up_n else None},
            "down": {"n": down_n, "accuracy": round(down_correct / down_n, 5) if down_n else None},
        },
        "fold_consistency": round(
            sum(1 for f in fold_reports
                if (f["accuracy_independent"] or 0) > threshold) / len(fold_reports), 4
        ),
    }
...
def classify_edge(report: dict, cfg: Config, min_rows: int) -> dict[str, Any]:
    """Promuove solo un vantaggio ECONOMICO su OOS non sovrapposto."""
    ...
    if not n or acc is None or not ci:
        return {"classification": EDGE_INCONCLUSIVE,
                "notes": ["nessun risultato OOS indipendente"]}
    needed = max(int(min_rows), int(cfg.ml_min_independent_oos))
    if n < needed:
        notes.append(f"solo {n} prove OOS non sovrapposte (ne servono {needed})")
        return {"classification": EDGE_INCONCLUSIVE, "notes": notes,
                "out_of_sample": oos}

    threshold = breakeven_win_rate(cfg.payout) if cfg.payout is not None else 0.5
    lo = float(ci[0])
    p = oos.get("p_value_vs_breakeven")
    p = 1.0 if p is None else float(p)
    consistency = float(report.get("fold_consistency") or 0.0)
    baseline = float(oos.get("baseline_accuracy") or 0.5)
    excess = float(oos.get("excess_vs_baseline") or 0.0)
    if cfg.payout is not None:
        notes.append(f"payout {cfg.payout:g}: break-even {threshold:.4f}")
    else:
        notes.append("payout sconosciuto: soglia statistica 0.5000")
    notes.append(f"baseline temporale {baseline:.4f}; vantaggio modello {excess:+.4f}")

    beats_baseline = excess >= cfg.ml_min_excess_vs_baseline
    if (lo > threshold and p < 0.01 and consistency >= 0.8 and beats_baseline):
        cls = EDGE_PROVEN
        notes.append(f"limite inferiore {lo:.4f} sopra break-even, p={p:.4g}, "
                     f"{consistency:.0%} fold positivi")
    elif (lo > threshold and p < 0.05 and consistency >= 0.6 and beats_baseline):
        cls = EDGE_PROMISING
        notes.append(f"vantaggio promettente: limite inferiore {lo:.4f}, p={p:.4g}, "
                     f"{consistency:.0%} fold positivi")
    else:
        cls = EDGE_NONE
        notes.append(f"accuratezza {float(acc):.4f}, limite inferiore {lo:.4f}, "
                     f"p_vs_break-even={p:.4g}, excess={excess:+.4f}")
    notes.append("Il verdetto usa solo finestre OOS distanziate >=60s; la metrica "
                 "dense resta diagnostica e non puo' promuovere un modello.")
...
def train_and_validate(store: Store, cfg: Config, include_synthetic: bool = False,
    ...
    report: dict[str, Any] = {
        ...
        "horizon_s": cfg.horizon_s, "dataset": desc,
        "include_synthetic": include_synthetic, "payout": cfg.payout,
        "payout_status": "NOTO" if cfg.payout is not None else "PAYOUT SCONOSCIUTO",
        "validation_policy": "purged walk-forward + OOS non-overlap >= horizon",
    }
    if include_synthetic:
        report["warning"] = "DATI SINTETICI: non possono dimostrare un edge reale."
    duration = float(desc.get("duration_minutes") or 0.0)
    if len(ds) < cfg.ml_min_samples or duration < cfg.ml_min_duration_minutes:
        report["status"] = "DATI INSUFFICIENTI"
        report["conclusion"] = (
            f"dataset 60s: {len(ds)} righe campionate, {duration:.1f} minuti. "
            f"Servono almeno {cfg.ml_min_samples} righe e "
            f"{cfg.ml_min_duration_minutes:.0f} minuti reali prima di cercare un edge."
        )
        return report
    ...
    report["edge"] = classify_edge(wf, cfg, min_rows=cfg.ml_min_independent_oos)
    report["strategy_research"] = discover_strategies(ds, cfg) if cfg.research_enabled else {"enabled": False}
...
# --------------------------------------------------------------------------- #
#  LABORATORIO STRATEGIE M60: alpha simbolici, FDR, holdout fresco
# --------------------------------------------------------------------------- #

def _qlinear(values: list[float], q: float) -> float | None:
...
def _rule_prediction(rule: dict[str, Any], fmap: dict[str, float]) -> int | None:
    """Restituisce 1=UP, 0=DOWN, None=NO_TRADE per una regola scoperta.

    La grammatica e' volutamente piccola: code di una feature e accordo fra due
    feature. E' abbastanza espressiva per scoprire momentum/reversion/flow
    conditionali, ma ogni strategia resta leggibile e riproducibile.
    """
    a = fmap.get(str(rule.get("feature_a")))
    if not isinstance(a, (int, float)) or not math.isfinite(float(a)):
        return None
    a = float(a)
    orientation = int(rule.get("orientation", 1))
    up = 1 if orientation > 0 else 0
    down = 1 - up
    kind = rule.get("kind")
    if kind == "single_tail":
        lo = float(rule["low_a"]); hi = float(rule["high_a"])
        if a >= hi:
            return up
        if a <= lo:
            return down
        return None
    if kind == "pair_agree":
        b = fmap.get(str(rule.get("feature_b")))
        if not isinstance(b, (int, float)) or not math.isfinite(float(b)):
            return None
        b = float(b)
        la, ha = float(rule["low_a"]), float(rule["high_a"])
        lb, hb = float(rule["low_b"]), float(rule["high_b"])
        if a >= ha and b >= hb:
            return up
        if a <= la and b <= lb:
            return down
...
def _rule_score(rule: dict[str, Any], ds: Dataset, idxs: list[int]) -> dict[str, Any]:
    name_to_i = {n: i for i, n in enumerate(ds.names)}
    wins = losses = up_n = down_n = 0
    preds: dict[int, int] = {}
    for j in idxs:
        fmap = {n: ds.rows[j][i] for n, i in name_to_i.items()}
        pred = _rule_prediction(rule, fmap)
        if pred is None:
            continue
        preds[j] = pred
        truth = ds.y[j]
        wins += int(pred == truth)
        losses += int(pred != truth)
        up_n += int(pred == 1)
        down_n += int(pred == 0)
    n = wins + losses
    wr = wins / n if n else None
    return {"n": n, "wins": wins, "losses": losses, "win_rate": wr,
            "up_signals": up_n, "down_signals": down_n, "predictions": preds}


def _bh_fdr(pvals: list[tuple[int, float]], alpha: float) -> set[int]:
    """Benjamini-Hochberg: controlla il false discovery rate nella ricerca."""
    if not pvals:
        return set()
    ...
    for rank, (_, p) in enumerate(ordered, 1):
        if p <= alpha * rank / m:
            kmax = rank
    return {idx for idx, _ in ordered[:kmax]} if kmax > 0 else set()


def _strategy_overlap(a: dict[int, int], b: dict[int, int]) -> float:
    both = set(a).intersection(b)
    if not both:
        return 0.0
    return sum(int(a[j] == b[j]) for j in both) / len(both)


def discover_strategies(ds: Dataset, cfg: Config) -> dict[str, Any]:
    """Scopre specialisti M60 senza usare il futuro per costruire la regola.

    1) comprime il dataset a prove non sovrapposte (>=60s);
    2) discovery 50%: seleziona feature e soglie;
    3) validation 25%: confronta TUTTI i candidati e applica BH-FDR;
    4) holdout 25%: esame finale mai usato ne' per creare ne' per scegliere;
    5) conserva pochi specialisti diversi fra loro.
    """
    if not cfg.research_enabled or not ds.rows or not ds.names:
        return {"enabled": bool(cfg.research_enabled), "status": "DISABILITATO"}
    spacing = max(int(ds.horizon_s * 1000), int(cfg.ml_oos_spacing_ms))
    independent = _spaced_indices(ds.ts, 0, len(ds), spacing)
    if len(independent) < 80:
        return {"enabled": True, "status": "DATI INSUFFICIENTI",
                "independent_rows": len(independent),
                "note": "servono almeno 80 finestre M60 non sovrapposte"}

    n = len(independent)
    d_end = max(30, int(n * 0.50))
    v_end = max(d_end + 20, int(n * 0.75))
    v_end = min(v_end, n - 15)
    disc = independent[:d_end]
    val = independent[d_end:v_end]
    hold = independent[v_end:]
    if len(val) < 15 or len(hold) < 15:
        return {"enabled": True, "status": "DATI INSUFFICIENTI",
                "independent_rows": n}

    # Ranking univariato SOLO sul discovery set: differenza standardizzata fra
    # classi. Nessuna informazione di validation/holdout entra nella formula.
    ranked: list[tuple[float, str, int]] = []
    for i, name in enumerate(ds.names):
        u = [ds.rows[j][i] for j in disc if ds.y[j] == 1 and math.isfinite(ds.rows[j][i])]
        d = [ds.rows[j][i] for j in disc if ds.y[j] == 0 and math.isfinite(ds.rows[j][i])]
        if len(u) < 8 or len(d) < 8:
            continue
        mu, md = sum(u)/len(u), sum(d)/len(d)
        allv = u + d
        m = sum(allv)/len(allv)
        sd = math.sqrt(sum((x-m)**2 for x in allv) / max(len(allv)-1, 1))
        if sd <= 1e-12:
            continue
        ranked.append((abs(mu-md)/sd, name, i))
    ranked.sort(reverse=True)
    top = ranked[:max(2, int(cfg.research_top_features))]

    candidates: list[dict[str, Any]] = []
    # Due ampiezze di coda producono strategie piu' selettive o piu' frequenti.
    for _, name, i in top:
        vals = [ds.rows[j][i] for j in disc if math.isfinite(ds.rows[j][i])]
        for qlo, qhi in ((0.30, 0.70), (0.20, 0.80)):
            lo, hi = _qlinear(vals, qlo), _qlinear(vals, qhi)
            if lo is None or hi is None or hi <= lo:
                continue
            for orientation in (1, -1):
                candidates.append({"kind": "single_tail", "feature_a": name,
                                   "low_a": lo, "high_a": hi,
                                   "orientation": orientation})

    pair_top = top[:max(2, int(cfg.research_pair_features))]
    for x in range(len(pair_top)):
        for y in range(x+1, len(pair_top)):
            _, na, ia = pair_top[x]; _, nb, ib = pair_top[y]
            va = [ds.rows[j][ia] for j in disc if math.isfinite(ds.rows[j][ia])]
            vb = [ds.rows[j][ib] for j in disc if math.isfinite(ds.rows[j][ib])]
            la, ha = _qlinear(va, .35), _qlinear(va, .65)
            lb, hb = _qlinear(vb, .35), _qlinear(vb, .65)
            if None in (la, ha, lb, hb) or ha <= la or hb <= lb:
                continue
            for orientation in (1, -1):
                candidates.append({"kind": "pair_agree", "feature_a": na,
                                   "feature_b": nb, "low_a": la, "high_a": ha,
                                   "low_b": lb, "high_b": hb,
                                   "orientation": orientation})
    candidates = candidates[:max(1, int(cfg.research_max_candidates))]

    be = breakeven_win_rate(cfg.payout) if cfg.payout is not None else 0.5
    pvals: list[tuple[int, float]] = []
    evaluated: list[dict[str, Any]] = []
    for k, rule in enumerate(candidates):
        vs = _rule_score(rule, ds, val)
        if vs["n"] < cfg.research_min_validation_signals:
            continue
        p = binomial_upper_p_value(vs["wins"], vs["n"], be)
        rec = {**rule, "candidate_index": k, "validation": {
            kk: vv for kk, vv in vs.items() if kk != "predictions"
        }, "validation_p": p}
        evaluated.append(rec)
        pvals.append((len(evaluated)-1, p))

    passed_idx = _bh_fdr(pvals, float(cfg.research_fdr_alpha))
    survivors: list[dict[str, Any]] = []
    for idx in sorted(passed_idx):
        rule = evaluated[idx]
        hs = _rule_score(rule, ds, hold)
        if hs["n"] < cfg.research_min_holdout_signals:
            continue
        wr = float(hs["win_rate"] or 0.0)
        p_hold = binomial_upper_p_value(hs["wins"], hs["n"], be)
        margin = wr - be
        # EV per una puntata unitaria nel prodotto binario.
        payout = float(cfg.payout) if cfg.payout is not None else 1.0
        ev = wr * payout - (1.0 - wr)
        if (margin < cfg.research_min_margin_over_be or ev <= 0 or
                p_hold > cfg.research_holdout_p_max):
            continue
        stable_blocks: list[float] = []
        mid = len(hold)//2
        for block in (hold[:mid], hold[mid:]):
            bs = _rule_score(rule, ds, block)
            if bs["n"] >= 4 and bs["win_rate"] is not None:
                stable_blocks.append(float(bs["win_rate"]))
        # Una strategia non viene bocciata per un blocco piccolo, ma se entrambi
        # esistono almeno uno non deve essere chiaramente invertito (<50%).
        if stable_blocks and min(stable_blocks) < 0.50:
            continue
        desc = (f"{rule['feature_a']} code" if rule["kind"] == "single_tail" else
                f"{rule['feature_a']} + {rule['feature_b']} concordi")
        fingerprint = hashlib.sha1(json.dumps(rule, sort_keys=True, default=str).encode()).hexdigest()[:12]
        out = {k: v for k, v in rule.items() if k not in ("candidate_index",)}
        out.update({
            "strategy_id": f"alpha60_{fingerprint}", "description": desc,
            "validation": rule["validation"],
            "validation_p": round(float(rule["validation_p"]), 8),
            "holdout": {kk: vv for kk, vv in hs.items() if kk != "predictions"},
            "holdout_p": round(p_hold, 8), "margin_over_be": round(margin, 6),
            "ev_units": round(ev, 6), "stability_blocks": [round(x,4) for x in stable_blocks],
            # peso grezzo live: piccolo e limitato; la strategia e' specialista,
            # non un nuovo padrone del sistema.
            "live_weight": round(min(1.0, max(0.05, margin * 5.0)), 4),
            "_hold_predictions": hs["predictions"],
        })
        survivors.append(out)

    survivors.sort(key=lambda r: (r["ev_units"], r["holdout"]["n"]), reverse=True)
    selected: list[dict[str, Any]] = []
    for cand in survivors:
        if any(_strategy_overlap(cand["_hold_predictions"], x["_hold_predictions"]) > 0.90
               for x in selected):
            continue
        selected.append(cand)
        if len(selected) >= cfg.research_max_live_experts:
            break
    for x in selected:
        x.pop("_hold_predictions", None)
    for x in survivors:
        x.pop("_hold_predictions", None)

    return {
        "enabled": True, "status": "COMPLETO", "validation_policy": "grammar+BH-FDR+fresh-holdout",
        "independent_rows": n, "split": {"discovery": len(disc), "validation": len(val), "holdout": len(hold)},
        "features_ranked": [{"name": name, "effect": round(score,4)} for score,name,_ in top[:10]],
        "candidates_generated": len(candidates), "candidates_evaluated": len(evaluated),
        "fdr_survivors": len(passed_idx), "holdout_survivors": len(survivors),
        "selected": selected, "breakeven": round(be,6),
        "note": "Le strategie selezionate possono solo affiancare il modello M60; il peso live totale e' limitato."
    }


def strategy_library_prediction(model: Model, features: dict[str, Any], cfg: Config) -> dict[str, Any]:
    research = (model.metadata or {}).get("strategy_research") or {}
    rules = list(research.get("selected") or [])[:max(0, int(cfg.research_max_live_experts))]
    if not rules:
        return {"active": 0, "prob_up": None, "weight": 0.0, "votes": []}
    upw = downw = 0.0
    votes: list[dict[str, Any]] = []
    for rule in rules:
        pred = _rule_prediction(rule, features)
        if pred is None:
            continue
        w = max(0.01, float(rule.get("live_weight") or 0.05))
        if pred == 1: upw += w
        else: downw += w
        votes.append({"strategy_id": rule.get("strategy_id"),
                      "direction": UP if pred == 1 else DOWN, "weight": round(w,4)})
    total = upw + downw
    if total <= 0:
        return {"active": 0, "prob_up": None, "weight": 0.0, "votes": []}
    # Il peso complessivo resta un correttivo piccolo anche se molti specialisti
    # concordano: evita che alpha correlati aggirino il champion.
    cap = max(0.0, min(0.35, float(cfg.research_live_weight_cap)))
    weight = min(cap, 0.05 + 0.04 * len(votes))
    return {"active": len(votes), "prob_up": upw/total, "weight": weight, "votes": votes}


def fit_final_model(ds: Dataset, cfg: Config, strategy_research: dict[str, Any] | None = None) -> Model:
    ...
    # La calibrazione e' l'ultimo 20% e NON viene accorciata dalla purga.
    # Si arretra invece la fine del training finche' fra training e calibrazione
    # resta horizon+embargo. Questo e' lo stesso schema causale del walk-forward.
    cal_start = max(1, int(n * 0.8))
    cal_rows = n - cal_start
    train_cutoff_ts = ds.ts[cal_start] - gap_ms if cal_start < n else ds.ts[-1] - gap_ms
    train_end = bisect.bisect_right(ds.ts, train_cutoff_ts, lo=0, hi=cal_start)
    can_calibrate = (
        cal_rows >= 500 and train_end >= 500
        and len(set(ds.y[:train_end])) == 2
        and len(set(ds.y[cal_start:])) == 2
    )

    if can_calibrate:
        w, b, mean, std = fit_logistic(ds.rows[:train_end], ds.y[:train_end], cfg)
        model = Model(ds.names, mean, std, w, b, ds.horizon_s)
        scores = [model._logit(r) for r in ds.rows[cal_start:]]
        model.platt = _fit_platt(scores, ds.y[cal_start:])
        model.calibrated = True
        model.metadata = {"train_rows": train_end, "calibration_rows": cal_rows,
                          "purged_rows": cal_start - train_end,
                          "purge_gap_ms": gap_ms,
                          "validation_policy": MODEL_VALIDATION_POLICY,
                          "engine_version": VERSION,
                          "target_horizon_s": float(ds.horizon_s)}
    else:
        w, b, mean, std = fit_logistic(ds.rows, ds.y, cfg)
        model = Model(ds.names, mean, std, w, b, ds.horizon_s)
        model.calibrated = False
        model.metadata = {"train_rows": n,
                          "calibration": (f"saltata: train={train_end}, "
                                          f"coda={cal_rows}, gap={gap_ms}ms"),
                          "validation_policy": MODEL_VALIDATION_POLICY,
                          "engine_version": VERSION,
                          "target_horizon_s": float(ds.horizon_s)}
    if ds.ts:
        model.metadata.update({
            "source_min_ts": int(ds.ts[0]),
            "source_max_ts": int(ds.ts[-1]),
            "dataset_rows": int(len(ds)),
            "fresh_start": True,
            "optimizer_initial_weights": "zero",
        })
    if strategy_research is None and cfg.research_enabled:
        strategy_research = discover_strategies(ds, cfg)
    if strategy_research:
        model.metadata["strategy_research"] = strategy_research
    model.model_id = f"logistic_h{ds.horizon_s:g}s_{now_ms()}"
    return model


def continual_challenger_report(active: Model | None, ds: Dataset, cfg: Config) -> dict[str, Any]:
    """Confronta il nuovo apprendimento con il champion su futuro MAI visto.

    Il challenger impara anche dagli errori recenti, ma l'ultimo blocco resta
    completamente fuori dal suo training. In questo modo un errore puo' entrare
    nel giro successivo come feedback senza essere poi usato anche per giudicare
    lo stesso modello che l'ha memorizzato.
    """
    if active is None:
        return {"required": False, "ready": True, "passed": True,
                "reason": "nessun champion: vale la validazione walk-forward"}
    if not ds.ts:
        return {"required": True, "ready": False, "passed": False,
                "reason": "dataset vuoto"}
    meta = active.metadata or {}
    active_cut = int(meta.get("source_max_ts") or 0)
    if active_cut <= 0:
        return {"required": True, "ready": False, "passed": False,
                "reason": "champion privo del timestamp di training"}
    latest = int(ds.ts[-1])
    fresh_ms = max(0, latest - active_cut)
    need_fresh_ms = int(cfg.ml_challenger_fresh_minutes * 60000)
    if fresh_ms < need_fresh_ms:
        return {
            "required": True, "ready": False, "passed": False,
            "fresh_minutes": round(fresh_ms / 60000.0, 1),
            "needed_fresh_minutes": cfg.ml_challenger_fresh_minutes,
            "reason": (f"servono {cfg.ml_challenger_fresh_minutes:.0f} minuti "
                       f"mai visti dal champion; disponibili {fresh_ms/60000:.1f}"),
        }

    eval_ms = int(cfg.ml_challenger_eval_minutes * 60000)
    eval_start_ts = max(active_cut + 1, latest - eval_ms)
    gap_ms = int((ds.horizon_s + cfg.ml_embargo_s) * 1000)
    train_cutoff_ts = eval_start_ts - gap_ms
    train_end = bisect.bisect_right(ds.ts, train_cutoff_ts)
    eval_start = bisect.bisect_left(ds.ts, eval_start_ts)
    if train_end < 500 or len(set(ds.y[:train_end])) < 2:
        return {"required": True, "ready": False, "passed": False,
                "reason": f"training challenger insufficiente ({train_end} righe)"}

    w, b, mean, std = fit_logistic(ds.rows[:train_end], ds.y[:train_end], cfg,
                                   seed=1701 + (latest // 60000) % 100000)
    challenger = Model(ds.names, mean, std, w, b, ds.horizon_s)
    idxs = _spaced_indices(ds.ts, eval_start, len(ds),
                           max(int(ds.horizon_s * 1000), int(cfg.ml_oos_spacing_ms)))
    # Per sicurezza il duello usa solo timestamp successivi al training del champion.
    idxs = [j for j in idxs if ds.ts[j] > active_cut]
    if len(idxs) < cfg.ml_challenger_min_oos:
        return {
            "required": True, "ready": False, "passed": False,
            "fresh_minutes": round(fresh_ms / 60000.0, 1),
            "oos": len(idxs), "needed_oos": cfg.ml_challenger_min_oos,
            "reason": f"solo {len(idxs)} prove fresche per il duello",
        }

    champion_correct = challenger_correct = 0
    fixes = regressions = same_wrong = 0
    for j in idxs:
        truth = ds.y[j]
        fmap = {name: ds.rows[j][i] for i, name in enumerate(ds.names)}
        arow: list[float] = []
        for i, name in enumerate(active.names):
            v = fmap.get(name)
            if not isinstance(v, (int, float)) or not math.isfinite(float(v)):
                v = active.mean[i]
            arow.append(float(v))
        ap = 1 if active.predict_row(arow) >= 0.5 else 0
        cp = 1 if challenger.predict_row(ds.rows[j]) >= 0.5 else 0
        ac = ap == truth
        cc = cp == truth
        champion_correct += int(ac)
        challenger_correct += int(cc)
        if (not ac) and cc:
            fixes += 1
        elif ac and (not cc):
            regressions += 1
        elif (not ac) and (not cc):
            same_wrong += 1
    ...
    be = breakeven_win_rate(cfg.payout) if cfg.payout is not None else 0.5
    passed = (challenger_acc >= be and
              improvement >= cfg.ml_challenger_min_improvement and
              fixes > regressions)
    return {
        "required": True, "ready": True, "passed": passed, "oos": n,
        "fresh_minutes": round(fresh_ms / 60000.0, 1),
        "eval_minutes": cfg.ml_challenger_eval_minutes,
        "champion_id": active.model_id,
        "champion_accuracy": round(champion_acc, 5),
        "challenger_accuracy": round(challenger_acc, 5),
        "improvement": round(improvement, 5),
        "required_improvement": cfg.ml_challenger_min_improvement,
        "break_even": round(be, 5),
        "fixed_old_errors": fixes,
        "new_errors": regressions,
        "same_errors": same_wrong,
        "train_rows_challenger": train_end,
        "reason": ("challenger migliore sul futuro fresco" if passed else
                   "challenger non migliora abbastanza il champion sul futuro fresco"),
    }
...
class Retrainer:
    ...
    def __init__(self, cfg: Config, store: Store,
                 on_model: Callable[[Model], None] | None = None,
                 current_model: Callable[[], Model | None] | None = None) -> None:
        self.cfg = cfg
        self.store = store
        ...
        self.last_study_max_ts = int(
            self.store.get_learning_state("m60_last_study_max_ts", 0) or 0
        )
        self.last_study_signature = str(
            self.store.get_learning_state("m60_last_study_signature", "") or ""
        )
        self.generation = int(
            self.store.get_learning_state("m60_generation", 0) or 0
        )
    ...
    def run_once(self, include_synthetic: bool | None = None) -> dict[str, Any]:
        cfg = self.cfg
        include_synthetic = (include_synthetic if include_synthetic is not None else False)
        self.runs += 1
        self.last_error = None
        stats = self.store.feature_stats(include_synthetic)
        raw_rows = int(stats["rows"])
        span_ms = int(stats["span_ms"])
        max_ts = int(stats.get("max_ts") or 0)
        sampled_est = min(raw_rows, span_ms // max(cfg.ml_sample_interval_ms, 1) + 1)
        duration_min = span_ms / 60000.0
        self.usable_rows = sampled_est
        signature = f"{raw_rows}:{stats.get('min_ts')}:{max_ts}:{span_ms}"

        if signature == self.last_study_signature:
            self.last_run_ts = now_ms()
            self.last_result = {
                "skipped": "nessun dato nuovo: questo dataset e' gia' stato studiato",
                "dataset_signature": signature, "generation": self.generation,
                "note": "Non ripeto lo stesso addestramento sullo stesso identico errore."
            }
            return self.last_result

        if self.last_study_max_ts and max_ts > self.last_study_max_ts:
            new_min = (max_ts - self.last_study_max_ts) / 60000.0
            if new_min < cfg.ml_min_new_study_minutes:
                self.last_run_ts = now_ms()
                self.last_result = {
                    "skipped": (f"solo {new_min:.1f} minuti di dati nuovi; "
                                f"ne servono {cfg.ml_min_new_study_minutes:.0f} "
                                "prima di ristudiare"),
                    "new_minutes": round(new_min, 1), "generation": self.generation,
                    "note": "Accumulo feedback nuovo invece di rifare quasi lo stesso training."
                }
                return self.last_result

        active = self.current_model() if self.current_model else None
        if active is not None:
            active_cut = int((active.metadata or {}).get("source_max_ts") or 0)
            if active_cut:
                fresh_min = max(0.0, (max_ts - active_cut) / 60000.0)
                if fresh_min < cfg.ml_challenger_fresh_minutes:
                    self.last_run_ts = now_ms()
                    self.last_result = {
                        "skipped": (f"champion {active.model_id}: solo {fresh_min:.1f} "
                                    f"minuti mai visti; ne servono "
                                    f"{cfg.ml_challenger_fresh_minutes:.0f} per un challenger"),
                        "fresh_minutes": round(fresh_min, 1),
                        "generation": self.generation,
                        "note": "Il modello attivo resta invariato finche' non esiste un test futuro vero."
                    }
                    return self.last_result

        if (sampled_est < cfg.ml_min_samples or
                duration_min < cfg.ml_min_duration_minutes):
            self.last_run_ts = now_ms()
            self.last_result = {
                "skipped": (f"studio 60s non ancora maturo: ~{sampled_est} righe a 1Hz, "
                            f"{duration_min:.1f} minuti; servono {cfg.ml_min_samples} righe "
                            f"e {cfg.ml_min_duration_minutes:.0f} minuti"),
                "feature_rows_raw": raw_rows, "feature_rows_sampled_est": sampled_est,
                "duration_minutes": round(duration_min, 1), "needed": cfg.ml_min_samples,
                "generation": self.generation,
                "note": ("Le righe sintetiche sono escluse dalla ricerca dell'edge."
                         if not include_synthetic else None),
            }
            return self.last_result

        # Da qui in poi e' uno STUDIO VERO. Ogni fit_logistic ricomincia con w=0, b=0.
        ...
        if edge in (EDGE_PROVEN, EDGE_PROMISING):
            ds = build_dataset(self.store, cfg, cfg.horizon_s, include_synthetic,
                               max_rows=cfg.ml_max_rows)
            duel = continual_challenger_report(active, ds, cfg)
            if len(ds) >= 300 and duel.get("ready") and duel.get("passed"):
                model = fit_final_model(ds, cfg, report.get("strategy_research"))
                self.generation += 1
                model.metadata.update({
                    "learning_generation": self.generation,
                    "parent_model_id": active.model_id if active else None,
                    "dataset_signature": signature,
                    "challenger_report": duel,
                })
                ...
                self.store.set_learning_state("m60_generation", self.generation)
                if self.on_model:
                    self.on_model(model)
                activated = True
                self.activations += 1

        # Persisti anche gli studi NON promossi: dopo un riavvio non deve rifare
        # lo stesso identico esame e raccontarsi di nuovo lo stesso errore.
        self.last_study_max_ts = max_ts
        self.last_study_signature = signature
        self.store.set_learning_state("m60_last_study_max_ts", max_ts)
        self.store.set_learning_state("m60_last_study_signature", signature)
        self.store.set_learning_state("m60_last_study_result", {
            "ts": self.last_run_ts, "edge": edge, "activated": activated,
            "model_id": model_id, "generation": self.generation, "duel": duel,
            "strategy_research": report.get("strategy_research"),
        })

        if activated:
            note = ("Nuova generazione attivata: parte da zero come fit, ha imparato "
                    "anche dagli errori precedenti e ha battuto il champion su futuro fresco.")
        elif duel and duel.get("required") and not duel.get("passed"):
            note = ("Challenger NON attivato: non ha corretto abbastanza errori del "
                    "champion su dati mai visti. Il modello live resta quello migliore.")
        else:
            note = ("Nessun modello attivato: il giro non ha superato la validazione; "
                    "gli errori osservati restano nei dati e saranno training nei giri futuri.")
        ...
        self.last_result = {
            ...
            "generation": self.generation, "dataset_signature": signature,
            "challenger": duel,
            "strategy_research": report.get("strategy_research"),
            "note": note,
        }
    ...
    def status(self) -> dict[str, Any]:
        return {
            ...
            "sample_interval_ms": self.cfg.ml_sample_interval_ms,
            "min_duration_minutes": self.cfg.ml_min_duration_minutes,
            "oos_spacing_ms": self.cfg.ml_oos_spacing_ms,
            "first_run_delay_s": round(self.cfg.retrain_initial_delay_s),
            "generation": self.generation,
            "last_study_max_ts": self.last_study_max_ts or None,
            "min_new_study_minutes": self.cfg.ml_min_new_study_minutes,
            "challenger_fresh_minutes": self.cfg.ml_challenger_fresh_minutes,
            "challenger_eval_minutes": self.cfg.ml_challenger_eval_minutes,
            ...
            "policy": ("Ogni studio riparte da pesi zero ma usa tutta la memoria utile, "
                       "con priorita' ai dati recenti. Non ristudia lo stesso dataset. "
                       "Se esiste un champion, il challenger deve batterlo su un blocco "
                       "futuro mai visto prima di sostituirlo."),
        }


# --------------------------------------------------------------------------- #
#  POCKET OPTION: collegamento di SOLA LETTURA
# --------------------------------------------------------------------------- #
#
#  Perche' esiste questa sezione.
#
#  Aurum decide su Binance, dove il libro degli ordini e' pubblico e la
#  microstruttura si puo' misurare. Ma se poi si opera su un broker di opzioni
#  binarie, quello che conta e' se il segnale sia spendibile LA'. Sono due
#  domande diverse, e la seconda non si risponde guardando Binance:
#
#    * il prezzo del broker puo' divergere da quello del venue;
#    * il broker puo' rispondere lento proprio quando serve;
#    * il segnale puo' essere gia' vecchio quando arriva al momento di usarlo.
#
#  Il portale misura le tre cose e mette un VERDETTO su ogni segnale. Un
#  segnale che Aurum emette ma che il broker non potrebbe onorare non e' un
#  segnale: contarlo fra i buoni gonfia le statistiche e basta.
#
#  ---------------------------------------------------------------------------
#  NESSUN ORDINE VIENE MAI INVIATO.
#
#  In questo file non esiste una chiamata di acquisto, vendita o piazzamento
#  ordine verso alcun venue. Il collegamento legge prezzo, saldo e stato della
#  connessione. La puntata e la durata che compaiono nel registro sono
#  un'ANNOTAZIONE - servono a rendere confrontabili le righe con quelle del
#  broker - non un'istruzione a operare.
#  ---------------------------------------------------------------------------

#: La libreria del broker e' opzionale: senza, il motore resta quello che e' -
#: un file solo, libreria standard e basta - e il portale mostra il pannello
#: spento con il motivo. Non e' un errore da gestire, e' una funzione in piu'
#: che si accende se la libreria c'e'.
try:                                            # pragma: no cover - opzionale
    from pocketoptionapi_async import AsyncPocketOptionClient
    POCKET_LIB_ERROR: str | None = None
except Exception as _exc:                       # pragma: no cover - opzionale
    AsyncPocketOptionClient = None              # type: ignore[assignment]
    POCKET_LIB_ERROR = f"{type(_exc).__name__}: {_exc}"


#: Verdetti del filtro di eseguibilita'.
EXECUTABLE, NOT_EXECUTABLE, UNVERIFIED = "ESEGUIBILE", "NON_ESEGUIBILE", "NON_VERIFICATO"


class PocketLink:
    """Legge il broker su un thread suo, con il suo event loop.

    Il client del broker e' asincrono, il motore no: farli convivere nello
    stesso ciclo significherebbe o bloccare il feed di mercato mentre si
    aspetta il broker, o riscrivere il motore attorno a asyncio. Un thread
    dedicato costa una manciata di byte e tiene le due latenze separate - che
    e' esattamente cio' che il portale deve misurare.

    Tutto quello che il resto del programma vede e' `state()`: un dizionario
    che non blocca mai e che dice anche quando NON sa (prezzo None, errore
    valorizzato). Un portale che finge di sapere e' peggio di uno che tace.
    """
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self._state: dict[str, Any] = {
            "enabled": bool(cfg.pocket_enabled),
            "available": AsyncPocketOptionClient is not None,
            "connected": False,
            "asset": cfg.pocket_asset,
            "demo": bool(cfg.pocket_demo),
            "price": None,
            "price_ts": None,
            "balance": None,
            "currency": None,
            "query_ms": None,
            "error": (POCKET_LIB_ERROR if AsyncPocketOptionClient is None
                      else None),
            "reads": 0,
        }
        #: Storia dei due prezzi affiancati, per il grafico di divergenza.
    ...
    def price(self) -> float | None:
        with self._lock:
            return self._state.get("price")

    # ------------------------------------------------------------- avvio
    def start(self) -> None:
        if not self.cfg.pocket_enabled:
            self._set(enabled=False)
            return
        if AsyncPocketOptionClient is None:
            self._set(enabled=True, error=(
                "libreria 'pocketoptionapi_async' non installata: "
                "pip install pocketoptionapi-async. "
                f"({POCKET_LIB_ERROR})"))
            return
        self._running = True
        self._thread = threading.Thread(target=self._run, name="pocket",
                                        daemon=True)
        self._thread.start()
    ...
    # --------------------------------------------------------- lettura auth
    def _auth(self) -> str:
        path = self.cfg.pocket_auth_file
        if not path:
            raise ValueError(
                "serve --pocket-auth percorso/pocket.auth con la stringa di "
                "sessione del broker")
        ...
        if not auth.startswith('42["auth"'):
            raise ValueError(
                f"{path} non ha il formato atteso: deve cominciare con "
                '42["auth"')
        # Il file contiene una sessione: non finisce mai in un log, in una
        # risposta HTTP o in un evento. Qui se ne legge solo la forma.
        demo_flag = '"isDemo":1' in auth
        self._set(demo=demo_flag)
        if not demo_flag and self.cfg.pocket_demo:
            self._set(error=(
                "ATTENZIONE: la sessione e' LIVE ma il portale e' impostato su "
                "demo. Il collegamento resta comunque di sola lettura."))
        return auth
    ...
    def _run(self) -> None:
        try:
            import asyncio
        except Exception as exc:  # noqa: BLE001 - praticamente impossibile
            self._set(error=f"asyncio non disponibile: {exc}")
            return
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            loop.run_until_complete(self._loop())
        except Exception as exc:  # noqa: BLE001 - il portale non muore mai
            ...
            try:
                if client is None:
                    ...
                    client = AsyncPocketOptionClient(
                        auth, is_demo=self.cfg.pocket_demo,
                        enable_logging=False,   # la sessione non va nei log
                    )
                ...
                price, query_ms = await self._read_price(client)
                self._set(price=price, price_ts=now_ms(),
                          query_ms=round(query_ms, 2), connected=True,
                          error=None)
                with self._lock:
                    self._state["reads"] += 1
                ...
                if balance_countdown <= 0:
                    balance_countdown = 8      # il saldo cambia lentamente
                    bal, cur = await self._read_balance(client)
                    if bal is not None:
                        self._set(balance=bal, currency=cur)
            except Exception as exc:  # noqa: BLE001 - si riprova, non si muore
                self._set(connected=False, price=None,
                ...
                continue
            await asyncio.sleep(max(0.2, self.cfg.pocket_poll_s))
    ...
    async def _read_price(self, client) -> tuple[float | None, float]:
        """Ultima chiusura da un minuto. Il tempo di risposta E' un dato."""
        start = time.perf_counter()
        candles = await client.get_candles(self.cfg.pocket_asset, 60, count=3)
        query_ms = (time.perf_counter() - start) * 1000.0
        if not candles:
            return (None, query_ms)
        last = candles[-1]
        price = getattr(last, "close", None)
        if price is None and isinstance(last, dict):
            price = last.get("close")
        return ((float(price) if price is not None else None), query_ms)

    async def _read_balance(self, client) -> tuple[float | None, str | None]:
        try:
            result = await client.get_balance()
        except Exception:  # noqa: BLE001 - il saldo e' un di piu'
            return (None, None)
        ...
        return ((float(balance) if balance is not None else None), currency)

    # ------------------------------------------------- confronto dei prezzi
    def compare(self, aurum_price: float | None) -> dict[str, Any]:
        """Quanto distano i due prezzi, in punti base.

        E' la misura che decide se un segnale sia spendibile: due prezzi che
        divergono di piu' del movimento che ci si aspetta sull'orizzonte
        rendono la direzione irrilevante.
        """
        pocket = self.price
        out: dict[str, Any] = {
            "aurum_price": aurum_price, "pocket_price": pocket,
            "diff": None, "diff_bps": None, "coherent": None,
            "limit_bps": self.cfg.pocket_max_diff_bps,
        }
        if aurum_price and pocket and aurum_price > 0:
            diff = pocket - aurum_price
            out["diff"] = round(diff, 6)
            out["diff_bps"] = round(abs(diff) / aurum_price * 10_000.0, 4)
            out["coherent"] = out["diff_bps"] <= self.cfg.pocket_max_diff_bps
            self.history.append({"t": now_ms(), "aurum": aurum_price,
                                 "pocket": pocket})
        return out

    def curve(self, points: int = 300) -> list[dict[str, Any]]:
...
class PocketGate:
    """Il verdetto di eseguibilita' su un singolo segnale.

    Vale la pena dire cosa NON e': non e' un secondo motore decisionale e non
    puo' trasformare un segnale sbagliato in uno giusto. Risponde solo a
    "questo, adesso, la' sarebbe eseguibile?", e ogni motivo di rifiuto e' un
    fatto misurato, non un giudizio sul segnale.
    """

    def __init__(self, cfg: Config, link: PocketLink, store: Store | None):
        self.cfg = cfg
        self.link = link
        self.store = store
        self.counters: dict[str, int] = {EXECUTABLE: 0, NOT_EXECUTABLE: 0,
                                         UNVERIFIED: 0}
        self.reasons: dict[str, int] = {}
        self.last: dict[str, Any] | None = None

    def check(self, sig: Any, market: dict, health: dict) -> dict[str, Any]:
        cfg = self.cfg
        state = self.link.state()
        aurum_price = market.get("price")
        cmp_ = self.link.compare(aurum_price)
        reasons: list[str] = []

        if not state.get("enabled"):
            verdict = UNVERIFIED
            reasons.append("collegamento al broker spento (--pocket per accenderlo)")
        elif not state.get("available"):
            verdict = UNVERIFIED
            reasons.append("libreria del broker non installata")
        elif not state.get("connected"):
            verdict = NOT_EXECUTABLE
            reasons.append("broker non connesso")
        else:
            verdict = EXECUTABLE
            if state.get("price") is None:
                reasons.append("prezzo del broker non disponibile")
            query_ms = state.get("query_ms")
            if query_ms is None or query_ms > cfg.pocket_max_query_ms:
                reasons.append(
                    f"broker lento: {query_ms if query_ms is not None else '?'}ms "
                    f"oltre {cfg.pocket_max_query_ms:.0f}ms")
            diff_bps = cmp_.get("diff_bps")
            if diff_bps is None:
                reasons.append("prezzi non confrontabili")
            elif diff_bps > cfg.pocket_max_diff_bps:
                reasons.append(
                    f"prezzi divergenti: {diff_bps:.2f}bps oltre "
                    f"{cfg.pocket_max_diff_bps:.2f}bps")

        feed_age = (health.get("market") or {}).get("feed_age_ms")
        if feed_age is not None and feed_age > cfg.pocket_max_feed_age_ms:
            reasons.append(f"feed Aurum vecchio di {feed_age:.0f}ms")
            if verdict == EXECUTABLE:
                verdict = NOT_EXECUTABLE

        created = getattr(sig, "triggered_at", None) or getattr(sig, "created_at", 0)
        signal_age = now_ms() - int(created or 0)
        if signal_age > cfg.pocket_max_signal_age_ms:
            reasons.append(f"segnale gia' vecchio di {signal_age}ms")
            if verdict == EXECUTABLE:
                verdict = NOT_EXECUTABLE

        if verdict == EXECUTABLE and reasons:
            verdict = NOT_EXECUTABLE
        ...
        row = {
            ...
            "asset": cfg.pocket_asset,
            "aurum_price": aurum_price,
            "pocket_price": state.get("price"),
            "diff_bps": cmp_.get("diff_bps"),
            "feed_age_ms": feed_age,
            "query_ms": state.get("query_ms"),
            "signal_age_ms": signal_age,
            "verdict": verdict,
            "reason": "; ".join(reasons) if reasons else "tutti i controlli superati",
            "stake": cfg.pocket_stake,
            "duration_s": cfg.pocket_duration_s,
            "result": None,
            "is_synthetic": int(bool(getattr(sig, "is_synthetic", False))),
        }
        self.counters[verdict] = self.counters.get(verdict, 0) + 1
        for r in reasons:
            key = gate_key(r)
            self.reasons[key] = self.reasons.get(key, 0) + 1
        self.last = row
        if self.store is not None:
            self.store.upsert_pocket_check(row)
        return row

    def settle(self, signal_id: str, result: str | None) -> None:
        """L'esito di Aurum viene riportato sulla riga di verifica.

        Serve a rispondere alla domanda che conta: i segnali giudicati
        ESEGUIBILI hanno vinto piu' spesso di quelli scartati? Se no, il filtro
        sta costando invece di proteggere, e va detto."""
        if self.store is None or not signal_id:
            return
        self.store.set_pocket_result(signal_id, result)

    def status(self) -> dict[str, Any]:
        total = sum(self.counters.values())
        # La quota si calcola sui segnali che si e' POTUTO verificare. Con il
        # collegamento spento erano tutti "non verificati": dire 0% eseguibili
        # sarebbe una bocciatura di segnali che nessuno ha mai giudicato.
        judged = self.counters.get(EXECUTABLE, 0) + self.counters.get(NOT_EXECUTABLE, 0)
        return {
            **self.link.state(),
            "checked": total,
            "judged": judged,
            "counters": dict(self.counters),
            "executable_rate": (round(self.counters.get(EXECUTABLE, 0) / judged, 4)
                                if judged else None),
            "top_reasons": sorted(self.reasons.items(), key=lambda kv: -kv[1])[:6],
            "last": self.last,
            "limits": {
                "max_diff_bps": self.cfg.pocket_max_diff_bps,
                "max_query_ms": self.cfg.pocket_max_query_ms,
                "max_feed_age_ms": self.cfg.pocket_max_feed_age_ms,
                "max_signal_age_ms": self.cfg.pocket_max_signal_age_ms,
            },
            "stake": self.cfg.pocket_stake,
            "duration_s": self.cfg.pocket_duration_s,
            "note": ("SOLA LETTURA: il portale legge prezzo e saldo del broker "
                     "e giudica se il segnale sarebbe eseguibile. Nessun ordine "
                     "viene mai inviato: non esiste in questo file una chiamata "
                     "di acquisto o vendita."),
        }


def pocket_report(store: Store, include_synthetic: bool = True) -> dict[str, Any]:
    """I segnali giudicati eseguibili hanno vinto piu' spesso degli altri?

    E' l'unica domanda che dice se il filtro del broker valga il suo costo. Un
    filtro che scarta segnali vincenti non protegge: toglie.
    """
    conn = store.reader()
    try:
        where = "" if include_synthetic else " WHERE p.is_synthetic = 0"
        rows = [dict(r) for r in conn.execute(
            f"SELECT p.verdict, p.reason, p.diff_bps, p.result FROM pocket_checks p{where}"
        )]
    finally:
        conn.close()
    if not rows:
        return {"status": "NESSUN DATO",
                "note": "nessun segnale e' ancora passato dal filtro del broker"}
    out: dict[str, Any] = {"status": "OK", "checked": len(rows), "groups": []}
    for verdict in (EXECUTABLE, NOT_EXECUTABLE, UNVERIFIED):
        sel = [r for r in rows if r["verdict"] == verdict]
        decided = [r for r in sel if r["result"] in (WIN, LOSS)]
        wins = sum(1 for r in decided if r["result"] == WIN)
        group = {"verdict": verdict, "signals": len(sel), "decided": len(decided),
                 "wins": wins,
                 "win_rate": round(wins / len(decided), 4) if decided else None}
        if decided:
            lo, hi = wilson_interval(wins, len(decided))
            group["ci95"] = [round(lo, 4), round(hi, 4)]
        out["groups"].append(group)
    ok = next((g for g in out["groups"] if g["verdict"] == EXECUTABLE), None)
    ko = next((g for g in out["groups"] if g["verdict"] == NOT_EXECUTABLE), None)
    if ok and ko and ok["win_rate"] is not None and ko["win_rate"] is not None:
        out["delta_win_rate"] = round(ok["win_rate"] - ko["win_rate"], 4)
        out["verdetto"] = (
            "Il filtro del broker sta selezionando meglio del caso."
            if out["delta_win_rate"] > 0 else
            "ATTENZIONE: i segnali scartati hanno vinto quanto o piu' di quelli "
            "approvati. Questo filtro sta costando, non proteggendo.")
    else:
        out["verdetto"] = ("Servono esiti su entrambi i gruppi per dire se il "
                           "filtro protegga o costi.")
    return out
...
.ctrl input{width:110px;background:var(--panel2);color:var(--fg);
  border:1px solid var(--line);border-radius:8px;padding:9px;font-size:15px;
  text-align:center;font-family:var(--mono)}
.ctrl button{border:1px solid var(--line);background:var(--panel2);color:var(--fg);
  border-radius:8px;padding:9px 13px;cursor:pointer;font-weight:700;font-size:12px}
...
<div class="panel mb">
  <div class="ph">
    <div><h2>Portale broker &middot; eseguibilita'</h2>
      <div class="sub" id="pk-sub">il segnale sarebbe spendibile la' dove si opera?</div></div>
    <span class="pill" id="pk-state">—</span>
  </div>
  <div class="pb">
    <div class="metrics" id="pk-cards"></div>
    <div class="grid g2" style="margin-top:12px">
      <div>
        <div class="sub" style="margin-bottom:6px">Divergenza dei due prezzi</div>
        <canvas id="pk-chart" height="200"></canvas>
      </div>
      <div>
        <div class="sub" style="margin-bottom:6px">Puntata annotata sul registro</div>
        <div class="ctrl">
          <button onclick="pkStake(-1)">&minus;</button>
          <input id="pk-stake" type="number" min="0.01" step="1">
          <button onclick="pkStake(1)">+</button>
          <button onclick="pkSave()">SALVA</button>
          <span id="pk-msg" class="sub"></span>
        </div>
        <div class="sub" style="margin-top:8px">
          E' un'annotazione per rendere confrontabile il registro con il broker.
          <b>Nessun ordine viene mai inviato:</b> in questo programma non esiste
          una chiamata di acquisto o vendita.
        </div>
        <div id="pk-reasons" style="margin-top:12px"></div>
      </div>
    </div>
    <div style="margin-top:12px" id="pk-verdict"></div>
    <div style="margin-top:10px;overflow-x:auto">
      <table id="pk-table"><thead><tr>
        <th>Ora</th><th>Dir</th><th>Conf.</th><th>Aurum</th><th>Broker</th>
        <th class="n">Diff bps</th><th class="n">Feed ms</th><th class="n">Broker ms</th>
        <th class="n">Eta' seg.</th><th>Verdetto</th><th>Esito</th><th>Motivo</th>
...
<!-- ----------------------------------------------- AGENTI / RESEARCH LIVE -->
<div class="grid g2 mb">

  <div class="panel">
    <div class="ph">
      <div>
        <h2>Agenti adattivi</h2>
        <div class="sub">
          pesi e affidabilita' appresi dal risultato reale
        </div>
      </div>
    </div>

    <div class="pb" id="agent-learning">
      <div class="empty">in attesa</div>
    </div>
  </div>
  ...
  <div class="panel">
    <div class="ph">
      <div>
        ...
        <div class="sub">
          FDR + holdout + specialisti M60
        </div>
      </div>
    </div>

    <div class="pb" id="research-live">
      <div class="empty">in attesa</div>
    </div>
  </div>
...
function get(path){
  return fetch(API+path).then(function(r){
    return r.json().then(function(body){
      if(!r.ok) throw new Error(path+": "+(body.error||("HTTP "+r.status)));
      return body;
    });
  });
}
...
function renderHistory(h){
  // Database vecchi o un upsert visto a cavallo del flush possono avere
  // `status` gia' definitivo e `result` ancora nullo. Lo storico deve usare
  // entrambi, senza mostrare WAITING/ACTIVE come operazioni concluse.
  var closed = {WIN:1, LOSS:1, TIE:1, CANCELLED:1};
  var rows = (h.trades||[]).filter(function(t){
    return !!closed[t.result || t.status];
  });
  ...
  $("history").innerHTML = '<table><thead><tr>'
    ...
    + rows.map(function(t){
      ...
      return '<tr><td class="tnum muted">'+hhmm(t.triggered_at||t.ts)+'</td>'
        ...
        + '<td><span class="tag '+result+'">'+result+'</span></td>'
...
function pkSave(){
  var el = $("pk-stake"), msg = $("pk-msg");
  fetch("/pocket/stake", {method:"POST",
    headers:{"Content-Type":"application/json"},
    body: JSON.stringify({stake: Number(el.value)})})
   .then(function(r){ return r.json(); })
   .then(function(d){
      msg.textContent = d.ok ? "annotazione salvata (nessun ordine inviato)"
                             : (d.error || "errore");
      msg.className = d.ok ? "sub up" : "sub warn";
      if(d.ok){ pkStakeLocal = d.stake; el.value = Number(d.stake).toFixed(2); }
      setTimeout(function(){ msg.textContent = ""; }, 3000);
   })
   .catch(function(e){ msg.textContent = String(e); msg.className = "sub warn"; });
}

function pkDrawChart(curve){
  var cv = $("pk-chart"); if(!cv) return;
  ...
  if(!curve || curve.length < 2){
    cx.fillStyle = "#6d7a90"; cx.font = "12px system-ui";
    cx.textAlign = "center";
    cx.fillText("nessun confronto registrato: collegamento al broker spento", W/2, H/2);
    return;
  }
  var vals = [];
  curve.forEach(function(p){ vals.push(p.aurum); vals.push(p.pocket); });
  ...
  function line(key,color){
    ...
    curve.forEach(function(p,i){
      if(i===0) cx.moveTo(X(i),Y(p[key])); else cx.lineTo(X(i),Y(p[key]));
    });
    cx.stroke();
  }
  line("aurum", "#4c8dff");
  line("pocket", "#f0a12e");
  cx.font = "11px system-ui"; cx.textAlign = "left";
  cx.fillStyle = "#4c8dff"; cx.fillText("Aurum / venue", pad, 14);
  cx.fillStyle = "#f0a12e";   cx.fillText("Broker", pad+90, 14);
}

function renderPocket(pk){
  var st = $("pk-state");
  if(!pk.enabled){
    st.textContent = "spento"; st.className = "pill";
  } else if(!pk.available){
    st.textContent = "libreria mancante"; st.className = "pill warn";
  } else if(pk.connected){
    st.textContent = "collegato \u00b7 sola lettura"; st.className = "pill on";
  } else {
    st.textContent = "non connesso"; st.className = "pill off";
  }
  $("pk-sub").textContent = pk.enabled
    ? ("asset " + (pk.asset||"—") + " \u00b7 " + (pk.demo ? "demo" : "live")
       + " \u00b7 sola lettura, nessun ordine")
    : "collegamento spento: avvia con --pocket per verificare l'eseguibilita'";

  var cmp = pk.compare || {}, c = pk.counters || {};
  function card(k,v,note,cls){
    return '<div class="metric"><div class="k">'+esc(k)+'</div>'
      + '<div class="v tnum'+(cls?" "+cls:"")+'">'+v+'</div>'
      + '<div class="d">'+esc(note||"")+'</div></div>';
  }
  $("pk-cards").innerHTML =
      card("Prezzo Aurum", cmp.aurum_price!=null ? "$"+num(cmp.aurum_price) : "—", "venue")
    + card("Prezzo broker", cmp.pocket_price!=null ? "$"+num(cmp.pocket_price) : "—",
           pk.asset||"—")
    + card("Divergenza", cmp.diff_bps!=null ? cmp.diff_bps.toFixed(2)+" bps" : "—",
           "limite "+(cmp.limit_bps!=null?cmp.limit_bps:"—")+" bps",
           cmp.coherent===false ? "down" : (cmp.coherent ? "up" : ""))
    + card("Risposta broker", pk.query_ms!=null ? Math.round(pk.query_ms)+" ms" : "—",
           "lettura prezzo",
           (pk.query_ms!=null && pk.limits && pk.query_ms>pk.limits.max_query_ms)?"warn":"")
    + card("Saldo broker", pk.balance!=null ? num(pk.balance) : "—",
           pk.currency||"solo lettura")
    + card("Verificati", String(pk.judged||0) + " / " + String(pk.checked||0),
           pk.executable_rate!=null
             ? ("eseguibili "+pct(pk.executable_rate))
             : "nessuno giudicabile: collegamento spento",
           pk.executable_rate!=null && pk.executable_rate<0.5 ? "warn" : "");

  if(pkStakeLocal === null && pk.stake != null){
    pkStakeLocal = pk.stake;
    $("pk-stake").value = Number(pk.stake).toFixed(2);
  }

  var rs = pk.top_reasons || [];
  $("pk-reasons").innerHTML = rs.length
    ? '<div class="sub" style="margin-bottom:5px">Perche\u2019 vengono scartati</div>'
      + rs.map(function(r){
          return '<div class="kv"><span>'+esc(r[0])+'</span>'
            + '<span class="n tnum">'+r[1]+'</span></div>'; }).join("")
    : '';

  if(pk.error){
    $("pk-verdict").innerHTML =
      '<div class="empty warn">broker: '+esc(pk.error)+'</div>';
  } else {
    $("pk-verdict").innerHTML = '';
  }

  pkDrawChart(pk.curve);

  var body = $("pk-table").querySelector("tbody");
  var rows = pk.checks || [];
  if(!rows.length){
    body.innerHTML = '<tr><td colspan="12" class="muted">'
      + 'nessun segnale ancora verificato</td></tr>';
    return;
  }
  body.innerHTML = rows.map(function(r){
    var vcls = r.verdict==="ESEGUIBILE" ? "up"
             : (r.verdict==="NON_ESEGUIBILE" ? "down" : "muted");
    return '<tr>'
      + '<td class="tnum">'+hhmm(r.ts)+'</td>'
      + '<td class="'+(r.direction==="UP"?"up":"down")+'">'+esc(r.direction||"—")+'</td>'
      + '<td class="tnum">'+(r.confidence!=null?pct(r.confidence):"—")+'</td>'
      + '<td class="tnum">'+(r.aurum_price!=null?num(r.aurum_price):"—")+'</td>'
      + '<td class="tnum">'+(r.pocket_price!=null?num(r.pocket_price):"—")+'</td>'
      + '<td class="n tnum">'+(r.diff_bps!=null?r.diff_bps.toFixed(2):"—")+'</td>'
      + '<td class="n tnum">'+(r.feed_age_ms!=null?Math.round(r.feed_age_ms):"—")+'</td>'
      + '<td class="n tnum">'+(r.query_ms!=null?Math.round(r.query_ms):"—")+'</td>'
      + '<td class="n tnum">'+(r.signal_age_ms!=null?Math.round(r.signal_age_ms):"—")+'</td>'
      + '<td class="'+vcls+'">'+esc(r.verdict||"—")+'</td>'
      + '<td>'+(r.result?'<span class="tag '+esc(r.result)+'">'+esc(r.result)+'</span>':"—")+'</td>'
...
function tickSlow(){
  // Lo storico ha un fetch indipendente: prima bastava UN errore in health,
  // wallet, retrain o statistics per saltare l'intero Promise.all e lasciare
  // la tabella vuota anche se /paper-trades conteneva correttamente le righe.
  get("/paper-trades?limit=200").then(renderHistory).catch(function(e){
    $("history").innerHTML = '<div class="empty warn">storico non disponibile: '
      + esc(String(e)) + '</div>';
  });
  // Anche il portale broker ha il suo fetch: e' la parte che dipende da una
  // rete di terzi, ed e' proprio quella che non deve poter azzerare il resto
  // della pagina quando il broker non risponde.
  get("/pocket").then(renderPocket).catch(function(e){
    $("pk-state").textContent = "non raggiungibile";
    $("pk-state").className = "pill off";
  });
  Promise.all([get("/statistics"), get("/retrain"), get("/health"),
               get("/db"), get("/wallet"), get("/agents"), get("/research")])
    .then(function(r){
      var st=r[0], rt=r[1], hl=r[2], db=r[3], wal=r[4],
          ag=r[5], rs=r[6];
      renderWallet(wal);

      var hrs = ag.adaptive_hit_rates || {};
      var aws = ag.current_weights || {};
      var names = Object.keys(aws);

      $("agent-learning").innerHTML = names.length
        ? names.map(function(n){
            var h = hrs[n], w = aws[n];

            return '<div class="kv"><span>'
              + esc(n)
              + '</span><span class="tnum">peso '
              + esc(String(w))
              + ' · hit '
              + (h===undefined ? '—' : pct(h))
              + '</span></div>';
          }).join('')
        : '<div class="empty">nessun peso disponibile</div>';

      var live = rs.live || {};
      var study = rs.last_study || {};

      $("research-live").innerHTML = kv([
        ["Stato",
          rs.enabled ? "ATTIVO" : "SPENTO",
          rs.enabled ? "up" : "muted"],

        ["Generation",
          String(rs.generation || 0)],

        ["Champion",
          rs.active_model || "nessuno"],

        ["Specialisti live",
          String(live.active || 0)],
        ...
        ["Candidate ultimo studio",
          study.candidates_generated !== undefined
            ? String(study.candidates_generated)
            : "—"],

        ["FDR survivors",
          study.fdr_survivors !== undefined
            ? String(study.fdr_survivors)
            : "—"],

        ["Holdout survivors",
          study.holdout_survivors !== undefined
            ? String(study.holdout_survivors)
            : "—"],
      ]);
      ...
      // Gli altri pannelli possono degradarsi senza nascondere lo storico.
      console.warn("tickSlow", e);
    });
}
...
def make_http_server(engine: "Engine"):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _body(self) -> dict:
            """Corpo JSON di una POST, con un tetto: un client che manda un
            corpo enorme non deve poter riempire la memoria del motore."""
            try:
                length = min(64 * 1024, int(self.headers.get("Content-Length", 0)))
            except (TypeError, ValueError):
                return {}
            if length <= 0:
                return {}
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                elif path == "/pocket/stake":
                    # Cambia SOLO l'annotazione della puntata usata nel
                    # registro di verifica. Non invia niente a nessuno.
                    body = self._body()
                    try:
                        stake = round(float(body.get("stake")), 2)
                    except (TypeError, ValueError):
                        self._send({"ok": False,
                                    "error": "valore non numerico"}, 400)
                        return
                    if not 0.01 <= stake <= 10_000.0:
                        self._send({"ok": False,
                                    "error": "fuori intervallo 0.01 - 10000"}, 400)
                        return
                    engine.cfg.pocket_stake = stake
                    self._send({"ok": True, "stake": stake,
                                "note": "annotazione aggiornata: nessun ordine "
                                        "e' mai stato inviato"})
                else:
        ...
        def _route(self, path: str):
            ...
            if path == "/statistics":
                ...
                cycle_start = int(
                    engine.signals.wallet.cycle_start_ts
                )

                cycle_trades = [
                    t for t in trades
                    if int(t.get("ts") or 0) >= cycle_start
                ]
                ...
                current.update({
                    "scope": "CURRENT_CYCLE",
                    "cycle": engine.signals.wallet.cycle,
                    "cycle_started_ts": cycle_start,
                    "all_time": all_time,
                })
            ...
            if path == "/agents":
                last = engine.signals.last_decision

                return {
                    "regime": last.regime if last else None,
                    "adaptive_hit_rates": (
                        engine.signals.agent_hit_rates(
                            last.regime if last else None
                        )
                    ),
                    "current_weights": (
                        (last.detail or {}).get("weights", {})
                        if last else {}
                    ),
                    "outputs": [
                        o.to_dict()
                        for o in (
                            last.agents if last else []
                        )
                    ],
                    "score_aware_learning": True,
                },
            ...
            if path == "/research":
                ...
                live = (
                    (last.detail or {}).get(
                        "strategy_research_live"
                    )
                    if last else None
                )

                retrain_status = engine.retrainer.status()
                lr = retrain_status.get("last_result") or {}
            ...
            if path == "/pocket":
                gate = engine.signals.pocket_gate
                return {**gate.status(),
                        "compare": engine.signals.pocket.compare(
                            engine.market.snapshot().get("price")),
                        "curve": engine.signals.pocket.curve(),
                        "checks": (engine.store.pocket_checks(limit=60)
                                   if engine.store else []),
                        "server_ts": now_ms()},
            if path == "/pocket/report":
                return (pocket_report(engine.store) if engine.store
                        else {"status": "NESSUN DB"}),
            if path == "/db":
                st = engine.store
                return {"path": os.path.abspath(st.path) if st else cfg.db_path,
...
class Engine:
    def __init__(self, cfg: Config, store: Store | None = None) -> None:
        ...
        self.retrainer = Retrainer(
            cfg, self.store, on_model=self._activate_model,
    ...
    def _load_active_model(self) -> None:
        try:
            artifact = self.store.active_model_artifact(self.cfg.horizon_s)
            if artifact:
                candidate = Model.from_json(artifact)
                policy = (candidate.metadata or {}).get("validation_policy")
                if (abs(candidate.horizon_s - self.cfg.horizon_s) < 0.001
                        and policy == MODEL_VALIDATION_POLICY):
                    self.model = candidate
                elif abs(candidate.horizon_s - self.cfg.horizon_s) < 0.001:
                    # Un vecchio h60 non e' automaticamente sicuro: le versioni
                    # precedenti potevano promuovere modelli usando test troppo
                    # sovrapposti. Lo ignoriamo e lasciamo che il nuovo retrainer
                    # lo rivalidi con OOS distanziato >= 60 secondi.
                    self.market.record_error(
                        "model",
                        f"modello legacy {candidate.model_id} ignorato: "
                        f"policy {policy or 'assente'} != {MODEL_VALIDATION_POLICY}",
                    )
    ...
    def _activate_model(self, model: Model) -> None:
        if abs(float(model.horizon_s) - float(self.cfg.horizon_s)) > 0.001:
            self._log(f"modello NON attivato: {model.model_id} e' h{model.horizon_s:g}s, "
                      f"il motore live e' h{self.cfg.horizon_s:g}s")
            return
        policy = (model.metadata or {}).get("validation_policy")
        if policy != MODEL_VALIDATION_POLICY:
            self._log(f"modello NON attivato: {model.model_id} ha policy "
                      f"{policy or 'assente'}, serve {MODEL_VALIDATION_POLICY}")
            return
        self.model = model
        self.signals.decisions.model = model
        self._log(f"modello 60s attivato: {model.model_id} "
    ...
    def start(self) -> None:
        ...
        self._running = True
        self.signals.pocket.start()
        pstate = self.signals.pocket.state()
        if pstate.get("enabled"):
            self._log(f"broker: collegamento di SOLA LETTURA a "
                      f"{self.cfg.pocket_asset}"
                      + (f" - {pstate['error']}" if pstate.get("error") else ""))
...
def cmd_run(cfg: Config, args) -> int:
    if cfg.strategy == "burst15":
        raise SystemExit(
            "BURST-15 a 5s e' disabilitato nel live di questa versione. "
            "Usa `burst` solo per analisi offline; `run` opera esclusivamente a 60s."
        )
...
def cmd_pocket(cfg: Config, args) -> int:
    """Il collegamento al broker: c'e', risponde, e i prezzi combaciano?

    E poi la domanda che conta davvero: i segnali giudicati eseguibili hanno
    vinto piu' spesso di quelli scartati? Un filtro che scarta segnali vincenti
    non protegge, toglie - e questo comando e' il modo di scoprirlo.
    """
    print(f"AURUM PORTALE {VERSION} - broker, SOLA LETTURA\n")
    if AsyncPocketOptionClient is None:
        print("  libreria del broker  : NON INSTALLATA")
        print(f"    {POCKET_LIB_ERROR}")
        print("    pip install pocketoptionapi-async")
    else:
        print("  libreria del broker  : presente")
    print(f"  asset                : {cfg.pocket_asset}")
    print(f"  file di sessione     : {cfg.pocket_auth_file or 'non impostato'}")
    print(f"  puntata annotata     : {cfg.pocket_stake:.2f} "
          f"su {cfg.pocket_duration_s}s")
    print(f"  limiti               : divergenza {cfg.pocket_max_diff_bps:g}bps, "
          f"risposta {cfg.pocket_max_query_ms:g}ms, "
          f"eta' segnale {cfg.pocket_max_signal_age_ms:g}ms")
    print("  ORDINI               : NON ESISTONO IN QUESTO PROGRAMMA")

    # La prova sul campo, se la libreria e la sessione ci sono.
    if cfg.pocket_enabled and AsyncPocketOptionClient is not None:
        link = PocketLink(cfg)
        link.start()
        deadline = time.time() + max(5.0, float(args.timeout))
        while time.time() < deadline and link.state().get("price") is None:
            time.sleep(0.25)
        st = link.state()
        link.stop()
        print()
        print(f"  connesso             : {'si' if st.get('connected') else 'NO'}")
        print(f"  prezzo               : {st.get('price')}")
        print(f"  risposta             : {st.get('query_ms')}ms")
        if st.get("error"):
            print(f"  errore               : {st['error']}")

    store = _open_existing(cfg) if os.path.exists(cfg.db_path) else None
    if store is not None:
        report = pocket_report(store)
        store.stop()
        print()
        print(json.dumps(report, indent=2, ensure_ascii=False, default=str))
    return 0
...
def cmd_selftest(cfg: Config, args) -> int:
    ...
    def t_learning() -> str:
        ...
        cfg_ml = Config(strategy="burst15", ml_epochs=12, horizon_s=5.0,
                        ml_oos_spacing_ms=5000, ml_min_independent_oos=50,
                        ml_min_duration_minutes=0.0, ml_min_samples=300)
        ...
        # Regressione del bug reale: a 60s + embargo derivato (120s), 6000
        # righe a 10Hz devono produrre fold e una coda calibrabile. Prima il
        # purge veniva tolto dal test e i fold diventavano zero.
        cfg_60 = Config(ml_epochs=2, horizon_s=60.0)
        folds_60 = walk_forward_folds(tss, 3, 60.0, cfg_60.ml_embargo_s)
        assert folds_60, "60s: purge ha eliminato tutti i fold"
        ds_60 = Dataset(rows, ys, tss, [100.0] * len(rows), [100.0] * len(rows),
                        ["signal", "noise"], 60.0)
        model_60 = fit_final_model(ds_60, cfg_60)
        assert model_60.calibrated, "60s: coda di calibrazione eliminata dal purge"
        ...
        ds2 = Dataset(rows2, ys2, tss, [100.0] * len(rows), [100.0] * len(rows),
                      ["a", "b"], 5.0)
        edge = classify_edge(run_walk_forward(ds2, cfg_ml, 3), cfg_ml, 50)
        assert edge["classification"] in (EDGE_NONE, EDGE_INCONCLUSIVE), \
    ...
    # ---------------------------------------------------- portale broker
    def t_pocket() -> str:
        """Il livello broker, senza la libreria del broker.

        Deve funzionare in tre modi diversi e dirlo: spento, libreria assente,
        collegato. E soprattutto NON deve mai inventare un prezzo che non ha.
        """
        import tempfile
        path = os.path.join(tempfile.mkdtemp(), "portale.db")
        c = Config(db_path=path, payout=0.8, pocket_enabled=False)
        store = Store(c)
        store.start()

        # 1. spento: verdetto NON VERIFICATO, mai NON ESEGUIBILE. Un
        #    collegamento che non c'e' non e' una bocciatura del segnale.
        link = PocketLink(c)
        gate = PocketGate(c, link, store)
        sig = LiveSignal(
            signal_id="p1", symbol="BTCUSDT", exchange="t", strategy="ensemble",
            direction=UP, status=ACTIVE, reference_price=100_000.0,
            trigger_price=0.0, horizon_s=60.0, confidence=0.7,
            prob_up=0.7, prob_down=0.3, edge=0.2, regime=TREND_UP,
            created_at=now_ms(), expires_wait_at=now_ms(),
        )
        market = {"price": 100_000.0}
        health = {"market": {"feed_age_ms": 40.0}}
        row = gate.check(sig, market, health)
        assert row["verdict"] == UNVERIFIED, row
        assert row["pocket_price"] is None, "prezzo inventato con il broker spento"

        # 2. acceso e coerente: eseguibile.
        c2 = Config(db_path=path, payout=0.8, pocket_enabled=True,
                    pocket_max_diff_bps=5.0)
        link2 = PocketLink(c2)
        link2._set(enabled=True, available=True, connected=True,
                   price=100_002.0, query_ms=120.0)
        gate2 = PocketGate(c2, link2, store)
        sig.signal_id = "p2"
        row = gate2.check(sig, market, health)
        assert row["verdict"] == EXECUTABLE, row
        assert row["diff_bps"] is not None and row["diff_bps"] < 5.0, row

        # 3. prezzi divergenti: NON eseguibile, con il numero nel motivo.
        link2._set(price=100_900.0)
        sig.signal_id = "p3"
        row = gate2.check(sig, market, health)
        assert row["verdict"] == NOT_EXECUTABLE, row
        assert "divergenti" in row["reason"], row["reason"]

        # 4. broker lento.
        link2._set(price=100_002.0, query_ms=5_000.0)
        sig.signal_id = "p4"
        row = gate2.check(sig, market, health)
        assert row["verdict"] == NOT_EXECUTABLE and "lento" in row["reason"], row

        # 5. il registro esiste davvero e l'esito ci torna sopra.
        gate2.settle("p2", WIN)
        gate2.settle("p3", LOSS)
        store.flush()
        rows = store.pocket_checks()
        assert len(rows) == 4, f"righe scritte: {len(rows)}"
        by_id = {r["signal_id"]: r for r in rows}
        assert by_id["p2"]["result"] == WIN, by_id["p2"]
        assert by_id["p2"]["verdict"] == EXECUTABLE, "l'esito ha sovrascritto il verdetto"
        report = pocket_report(store)
        store.stop()
        assert report["status"] == "OK", report
        assert report["checked"] == 4, report
        # E la garanzia che conta piu' di tutte: nel file non esiste una
        # chiamata capace di piazzare un ordine. Non e' una promessa nel
        # commento, e' una verifica che fallisce se qualcuno la aggiunge.
        # I nomi vengono composti a pezzi di proposito: cosi' nessuno di essi
        # compare letteralmente nel sorgente, e la verifica non deve escludere
        # se stessa con un ritaglio fragile del testo.
        forbidden = ("place" + "_order(", "create" + "_order(",
                     "send" + "_order(", "open" + "_trade(",
                     "place" + "_trade(", "execute" + "_order(",
                     ".buy" + "(", ".sell" + "(",
                     "client." + "buy", "client." + "sell")
        try:
            source = open(os.path.abspath(sys.modules[__name__].__file__),
                          encoding="utf-8").read()
        except Exception:  # noqa: BLE001 - eseguito da stdin o congelato
            source = ""
        if source:
            found = [tok for tok in forbidden if tok in source]
            assert not found, f"percorso d'ordine trovato nel file: {found}"
        return (f"4 verdetti ({row['verdict']} ultimo), registro coerente, "
                f"referto su {report['checked']} verifiche, "
                f"nessun percorso d'ordine nel file")

    # ------------------------------------------------- filtro del rumore
    def t_noise() -> str:
        """Due filtri che devono mordere, e in modo misurabile."""
        c = Config(signal_persistence_windows=3, min_move_over_noise=1.5)
        d = DecisionEngine(c)

        # Il rumore di fondo e' spread + un tick, in bps.
        floor = d._noise_floor_bps({"spread_bps": 0.4, "mid": 100_000.0,
                                    "tick_size": 0.01})
        assert abs(floor - 0.401) < 1e-3, floor
        assert d._noise_floor_bps({"spread_bps": None}) is None, \
            "senza spread il rumore non e' calcolabile: deve dirlo, non stimarlo"

        # La persistenza: tre finestre di fila, e una sola inversione azzera.
        assert d._persistent(UP, 1) == (False, 1)
        assert d._persistent(UP, 2) == (False, 2)
        assert d._persistent(UP, 3) == (True, 3)
        assert d._persistent(DOWN, 4) == (False, 1), \
            "un cambio di direzione deve azzerare la serie"
        assert d._persistent(UP, 5) == (False, 1)

        # E deve valere anche il confronto con l'orizzonte: una sigma sotto il
        # rumore per il fattore richiesto e' un NO TRADE, non un segnale debole.
        loose = Config(min_move_over_noise=1.5)
        assert 0.5 < loose.min_move_over_noise * 0.401 < 0.7, "aritmetica di soglia"
        return ("rumore = spread + tick; 3 finestre di fila richieste; "
                "un'inversione azzera la serie")
    ...
    def t_one_minute() -> str:
        ...
        with open(csv_path, "w", newline="") as fh:
            w = csv.writer(fh)
            w.writerow(["ts", "price", "quantity", "aggressor"])
            for i in range(6000):          # 10 minuti a 10 blocchi/s
        ...
        # 3. le annullate sono solo quelle ancora aperte quando il replay finisce.
        #    Con max_concurrent=2 possono essercene al massimo due.
        assert counters["cancelled"] <= c.max_concurrent, f"troppe annullate: {counters}"
    ...
    def t_continual_learning() -> str:
        import tempfile

        # 1) Lo stato dell'ultimo studio sopravvive a stop/restart: niente
        #    riaddestramento identico dopo un semplice riavvio del processo.
        path = os.path.join(tempfile.mkdtemp(), "continual.db")
        c = Config(db_path=path, payout=0.8)
        st = Store(c)
        st.set_learning_state("m60_last_study_signature", "sig-123")
        st.set_learning_state("m60_last_study_max_ts", 123456)
        st.set_learning_state("m60_generation", 4)
        st._conn.close()
        st2 = Store(c)
        assert st2.get_learning_state("m60_last_study_signature") == "sig-123"
        assert st2.get_learning_state("m60_last_study_max_ts") == 123456
        assert st2.get_learning_state("m60_generation") == 4
        st2._conn.close()
        os.unlink(path)

        # 2) Stesso dataset -> run_once deve saltare PRIMA di addestrare.
        class FakeStore:
            def __init__(self):
                self.state = {
                    "m60_last_study_signature": "10000:1:100000:99999",
                    "m60_last_study_max_ts": 100000,
                    "m60_generation": 2,
                }
            def get_learning_state(self, key, default=None):
                return self.state.get(key, default)
            def set_learning_state(self, key, value):
                self.state[key] = value
            def feature_stats(self, include_synthetic=True):
                return {"rows": 10000, "min_ts": 1, "max_ts": 100000,
                        "span_ms": 99999}
            def feature_rows(self, include_synthetic=True):
                return 10000
        rr = Retrainer(c, FakeStore())
        skipped = rr.run_once()
        assert "gia' stato studiato" in skipped.get("skipped", ""), skipped

        # 3) Champion/challenger: il nuovo modello viene promosso solo se
        #    corregge piu' errori di quanti ne introduca su futuro mai visto.
        rng = random.Random(77)
        rows, ys, tss = [], [], []
        t0 = 1_700_000_000_000
        for i in range(4 * 3600):  # quattro ore a 1 Hz
            x = rng.gauss(0, 1)
            rows.append([x])
            ys.append(1 if x > 0 else 0)
            tss.append(t0 + i * 1000)
        ds = Dataset(rows, ys, tss, [1.0]*len(rows), [1.0]*len(rows), ["x"], 60.0)
        active = Model(["x"], [0.0], [1.0], [-5.0], 0.0, 60.0,
                       calibrated=True, model_id="champion_bad",
                       metadata={
                           "validation_policy": MODEL_VALIDATION_POLICY,
                           "source_max_ts": tss[-1] - 90 * 60 * 1000,
                       })
        duel = continual_challenger_report(active, ds, c)
        assert duel.get("ready") and duel.get("passed"), duel
        assert duel["fixed_old_errors"] > duel["new_errors"], duel

        # 4) Un fit nuovo parte davvero da zero: stesso dataset+seed produce lo
        #    stesso modello anche dopo un fit precedente completamente diverso.
        a1 = fit_logistic(rows[:1200], ys[:1200], c, seed=11)
        _ = fit_logistic([[9.0], [-9.0]] * 200, [1, 0] * 200, c, seed=99)
        a2 = fit_logistic(rows[:1200], ys[:1200], c, seed=11)
        assert all(abs(x-y) < 1e-12 for x, y in zip(a1[0], a2[0]))
        assert abs(a1[1] - a2[1]) < 1e-12
        return (f"stato persistente, dataset duplicato saltato, challenger "
                f"{duel['challenger_accuracy']:.0%} vs champion "
                f"{duel['champion_accuracy']:.0%}, fit riparte da zero")
    ...
    check("apprendimento continuo (memoria, restart, challenger)", t_continual_learning)
    def t_strategy_research() -> str:
        c = Config(payout=0.8, research_enabled=True,
                   research_min_validation_signals=5,
                   research_min_holdout_signals=5,
                   research_fdr_alpha=0.20,
                   research_holdout_p_max=0.30,
                   research_min_margin_over_be=0.0)
        # Costruisce un alpha causale evidente: feature x con segno che anticipa
        # l'esito; rumore z non deve essere necessario. 240 punti distanziati 60s.
        rows=[]; y=[]; ts=[]; entry=[]; exit_=[]
        rng=random.Random(123)
        for i in range(240):
            truth = 1 if rng.random() > 0.5 else 0
            x = (2.0 if truth else -2.0) + rng.gauss(0, 0.35)
            z = rng.gauss(0, 1.0)
            rows.append([x,z]); y.append(truth); ts.append(1_700_000_000_000+i*60_000)
            entry.append(100.0); exit_.append(101.0 if truth else 99.0)
        ds=Dataset(rows,y,ts,entry,exit_,["alpha_x","noise_z"],60.0)
        rep=discover_strategies(ds,c)
        assert rep.get("selected"), rep
        m=fit_final_model(ds,c,rep)
        live=strategy_library_prediction(m,{"alpha_x":3.0,"noise_z":0.0},c)
        assert live.get("active",0)>=1 and live.get("prob_up") is not None
        return (f"{rep['candidates_generated']} candidati, "
                f"{len(rep['selected'])} specialisti promossi, voto live attivo")

    check("laboratorio strategie (FDR, holdout, specialisti)", t_strategy_research)

    check("statistica", t_stats)
    # Il vecchio end-to-end era specifico BURST a 5s. La v1.2 live e' solo 60s:
    # BURST e' gia' coperto dal test unitario e dal replay offline.
    ...
    check("portale broker (verdetto, registro, referto)", t_pocket)
    check("filtro del rumore (persistenza e soglia relativa)", t_noise)
...
def build_parser() -> argparse.ArgumentParser:
  ...
  %(prog)s burst --payout 0.8              analizza BURST offline; NON apre trade 5s
...
""")
    ...
    p.add_argument("--strategy", choices=["ensemble", "burst15"],
                   help="run usa solo ensemble 60s; burst15 e' mantenuto per replay offline")
    p.add_argument("--horizon", type=float,
                   help="compatibilita': nel live ensemble viene forzato a 60 secondi")
    ...
    p.add_argument("--pocket", action="store_true",
                   help="accende il collegamento di SOLA LETTURA al broker")
    p.add_argument("--pocket-auth",
                   help="file con la stringa di sessione del broker")
    p.add_argument("--pocket-asset", help="asset sul broker, es. BTCUSD")
    p.add_argument("--pocket-live", action="store_true",
                   help="usa la sessione live invece della demo (resta SOLA LETTURA)")
    p.add_argument("--pocket-stake", type=float,
                   help="puntata annotata nel registro di verifica")
    p.add_argument("--pocket-max-diff", type=float,
                   help="divergenza massima fra i due prezzi, in bps")
    p.add_argument("--persistence", type=int,
                   help="finestre consecutive con la stessa direzione prima di emettere")
    p.add_argument("--noise-factor", type=float,
                   help="quanto il movimento atteso deve battere il rumore di fondo")
...
def apply_args(cfg: Config, args) -> Config:
    ...
    if getattr(args, "pocket", False):
        cfg.pocket_enabled = True
    for attr, name in (("pocket_auth", "pocket_auth_file"),
                       ("pocket_asset", "pocket_asset"),
                       ("pocket_stake", "pocket_stake"),
                       ("pocket_max_diff", "pocket_max_diff_bps"),
                       ("persistence", "signal_persistence_windows"),
                       ("noise_factor", "min_move_over_noise")):
        value = getattr(args, attr, None)
        if value is not None:
            setattr(cfg, name, value)
    if getattr(args, "pocket_live", False):
        cfg.pocket_demo = False
    ...
    # Il database di default vive accanto allo script, non nella directory da
    # cui per caso e' stato lanciato Python. Altrimenti `python /x/aurum_engine.py`
    # da /home e `cd /x && python aurum_engine.py` aprono due aurum.db diversi:
    # il motore sembra perdere lo storico, mentre sta semplicemente leggendo
    # un altro file. Un --db esplicito resta invece esattamente quello scelto.
    if cfg.db_path == "aurum.db":
        cfg.db_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                   "aurum.db")
    cfg.__post_init__()
    return cfg
